...
#include "components/wootz_wallet/browser/json_rpc_service_test_utils.h"
#include "components/wootz_wallet/browser/pref_names.h"
#include "components/wootz_wallet/browser/sns_resolver_task.h"
#include "components/wootz_wallet/browser/unstoppable_domains_dns_resolve.h"
...
#include "components/decentralized_dns/core/constants.h"
...
void OnRequestResponse(bool* callback_called,
  ...
  *callback_called = true;
  ...
  bool success = error == wootz_wallet::mojom::ProviderError::kSuccess;
...
void OnStringResponse(bool* callback_called,
                      wootz_wallet::mojom::ProviderError expected_error,
                      const std::string& expected_error_message,
                      const std::string& expected_response,
                      const std::string& response,
                      wootz_wallet::mojom::ProviderError error,
...
void OnFilUint256Response(
    ...
    wootz_wallet::mojom::FilecoinProviderError expected_error,
    const std::string& expected_error_message,
    uint256_t expected_response,
    uint256_t response,
    wootz_wallet::mojom::FilecoinProviderError error,
    const std::string& error_message) {
  *callback_called = true;
  EXPECT_EQ(expected_response, response);
  EXPECT_EQ(expected_error, error);
...
class TestJsonRpcServiceObserver
    : public wootz_wallet::mojom::JsonRpcServiceObserver {
 public:
  ...
  ::mojo::PendingRemote<wootz_wallet::mojom::JsonRpcServiceObserver>
  GetReceiver() {
  ...
  mojo::Receiver<wootz_wallet::mojom::JsonRpcServiceObserver>
...
class JsonRpcServiceUnitTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    wootz_wallet::RegisterProfilePrefsForMigration(prefs_.registry());
  ...
  void SetInterceptor(const GURL& expected_url,
    ...
    url_loader_factory_.SetInterceptor(base::BindLambdaForTesting(
        [=, this](const network::ResourceRequest& request) {
          ...
          if (IsEndpointUsingWootzWalletProxy(request.url)) {
            EXPECT_TRUE(
                request.headers.GetHeader("x-wootz-key", &header_value));
            EXPECT_EQ(BUILDFLAG(WOOTZ_SERVICES_KEY), header_value);
          } else {
            EXPECT_FALSE(request.headers.HasHeader("x-wootz-key"));
          }
...
TEST_F(JsonRpcServiceUnitTest, SetNetwork) {
  ...
  for (const auto& network :
    ...
    EXPECT_EQ(url::Origin::Create(network_manager_->GetNetworkURL(
                  mojom::CoinType::ETH, origin_a)),
              url::Origin::Create(GURL("https://goerli-infura.wootz.com")));
    EXPECT_EQ(url::Origin::Create(network_manager_->GetNetworkURL(
                  mojom::CoinType::ETH, origin_b)),
              url::Origin::Create(GetActiveEndpointUrl(*network)));
  }
  ...
  EXPECT_EQ(
      ...
      url::Origin::Create(GURL("https://solana-mainnet.wallet.wootz.com")));
...
TEST_F(JsonRpcServiceUnitTest, AddEthereumChainApprovedForOrigin) {
  ...
  mojo::PendingRemote<wootz_wallet::mojom::JsonRpcServiceObserver> receiver;
  ...
  EXPECT_EQ("",
            json_rpc_service_->AddEthereumChainForOrigin(
                chain.Clone(), url::Origin::Create(GURL("https://chrome.com"))));
  json_rpc_service_->AddEthereumChainRequestCompleted("0x111", true);
  loop.Run();
          ...
          [&fourth_callback_is_called, &fourth_expected, &network_url](
            ...
            EXPECT_EQ(error_message,
                      l10n_util::GetStringFUTF8(
                          IDS_WOOTZ_WALLET_ETH_CHAIN_ID_FAILED,
...
TEST_F(JsonRpcServiceUnitTest, AddEthereumChainForOriginError) {
  ...
  auto origin = url::Origin::Create(GURL("https://chrome.com"));
  ...
  // New chain, not valid rpc url.
  {
    ...
    std::unique_ptr<TestJsonRpcServiceObserver> observer(
        new TestJsonRpcServiceObserver(
            ...
            l10n_util::GetStringFUTF8(IDS_WOOTZ_WALLET_ETH_CHAIN_ID_FAILED,
                                      base::ASCIIToUTF16(network_url.spec()))));
...
class UnstoppableDomainsUnitTest : public JsonRpcServiceUnitTest {
 public:
  ...
  std::string DnsWootzResponse() const {
    return MakeJsonRpcStringArrayResponse(
        {"", "", "", "", "", "https://chrome.com"});
  }
...
TEST_F(UnstoppableDomainsUnitTest, ResolveDns_PolygonNetworkError) {
  ...
  SetEthTimeoutResponse();
  SetPolygonTimeoutResponse();
  json_rpc_service_->UnstoppableDomainsResolveDns("wootz.crypto",
                                                  callback.Get());
  WaitAndVerify(&callback);
  ...
  EXPECT_CALL(callback,
              Run(std::optional<GURL>(), mojom::ProviderError::kInternalError,
                  l10n_util::GetStringUTF8(IDS_WALLET_INTERNAL_ERROR)));
  SetEthRawResponse(DnsWootzResponse());
  SetPolygonTimeoutResponse();
  json_rpc_service_->UnstoppableDomainsResolveDns("wootz.crypto",
                                                  callback.Get());
  WaitAndVerify(&callback);
...
TEST_F(UnstoppableDomainsUnitTest, ResolveDns_PolygonResult) {
  base::MockCallback<ResolveDnsCallback> callback;
  EXPECT_CALL(callback, Run(std::optional<GURL>("https://chrome.com"),
                            mojom::ProviderError::kSuccess, ""));
  SetEthTimeoutResponse();
  SetPolygonRawResponse(DnsWootzResponse());
  json_rpc_service_->UnstoppableDomainsResolveDns("wootz.crypto",
                                                  callback.Get());
  WaitAndVerify(&callback);
...
TEST_F(UnstoppableDomainsUnitTest, ResolveDns_InvalidDomain) {
  ...
  EXPECT_CALL(callback,
              Run(std::optional<GURL>(), mojom::ProviderError::kInvalidParams,
                  l10n_util::GetStringUTF8(IDS_WALLET_INVALID_PARAMETERS)));
  json_rpc_service_->UnstoppableDomainsResolveDns("wootz.test", callback.Get());
  EXPECT_EQ(0, url_loader_factory_.NumPending());
  task_environment_.RunUntilIdle();
}
...
TEST_F(UnstoppableDomainsUnitTest, ResolveDns_ManyCalls) {
  ...
  EXPECT_CALL(callback1, Run(std::optional<GURL>("https://chrome.com"),
                             mojom::ProviderError::kSuccess, ""));
  base::MockCallback<ResolveDnsCallback> callback2;
  EXPECT_CALL(callback2, Run(std::optional<GURL>("https://chrome.com"),
                             mojom::ProviderError::kSuccess, ""));
  ...
  // This will resolve wootz.crypto requests.
  eth_mainnet_getmany_call_handler_->AddItem(
      "wootz.crypto", keys[0],
      "QmbWqxBEKC3P8tqsKc98xmWNzrzDtRLMiMPL8wBuTGsMnR");
  eth_mainnet_getmany_call_handler_->AddItem("wootz.crypto", keys[5],
                                             "https://chrome.com");
  polygon_getmany_call_handler_->AddItem("wootz.crypto", keys[5],
                                         "https://chrome.com");

  // This will resolve wootz.x requests.
  polygon_getmany_call_handler_->AddItem(
      "wootz.x", keys[0], "QmbWqxBEKC3P8tqsKc98xmWNzrzDtRLMiMPL8wBuTGsMnR");
  polygon_getmany_call_handler_->AddItem("wootz.x", keys[5],
                                         "https://chrome.com");
  eth_mainnet_getmany_call_handler_->AddItem("wootz.x", keys[5],
                                             "https://chrome.com");
  ...
  json_rpc_service_->UnstoppableDomainsResolveDns("wootz.x", callback3.Get());
  task_environment_.RunUntilIdle();
  EXPECT_EQ(2, eth_mainnet_getmany_call_handler_->calls_number());
  EXPECT_EQ(2, polygon_getmany_call_handler_->calls_number());
  testing::Mock::VerifyAndClearExpectations(&callback3);
...
TEST_F(JsonRpcServiceUnitTest, UpdateIsEip1559CustomChain) {
  std::vector<base::Value::Dict> values;
  mojom::NetworkInfo chain1 = GetTestNetworkInfo1();
  values.push_back(wootz_wallet::NetworkInfoToValue(chain1));

  mojom::NetworkInfo chain2 = GetTestNetworkInfo2();
  values.push_back(wootz_wallet::NetworkInfoToValue(chain2));
  UpdateCustomNetworks(prefs(), &values);
...
TEST_F(JsonRpcServiceUnitTest, GetWalletAddrInvalidDomain) {
  const std::vector<std::string> invalid_domains = {"", ".eth", "-wootz.eth",
                                                    "wootz-.eth", "b.eth"};
...
TEST_F(JsonRpcServiceUnitTest, IsValidEnsDomain) {
  std::vector<std::string> valid_domains = {"wootz.eth", "test.wootz.eth",
                                            "wootz-test.test-dev.eth"};
  ...
  std::vector<std::string> invalid_domains = {
      "",      ".eth",    "-wootz.eth",      "wootz-.eth",     "wootz.e-th",
      "b.eth", "wootz.e", "-wootz.test.eth", "wootz-.test.eth"};
  for (const auto& domain : invalid_domains) {
...
TEST_F(JsonRpcServiceUnitTest, Reset) {
  std::vector<base::Value::Dict> values;
  mojom::NetworkInfo chain = GetTestNetworkInfo1("0x1");
  values.push_back(wootz_wallet::NetworkInfoToValue(chain));
  UpdateCustomNetworks(prefs(), &values);
  ...
  EXPECT_TRUE(prefs()->HasPrefPath(kWootzWalletEip1559CustomChains));
  EXPECT_TRUE(prefs()->HasPrefPath(kWootzWalletCustomNetworks));
  EXPECT_EQ(
  ...
  EXPECT_FALSE(prefs()->HasPrefPath(kWootzWalletCustomNetworks));
  EXPECT_EQ(
      network_manager_->GetCurrentChainId(mojom::CoinType::ETH, std::nullopt),
      mojom::kMainnetChainId);
  EXPECT_FALSE(prefs()->HasPrefPath(kWootzWalletEip1559CustomChains));
...
class ENSL2JsonRpcServiceUnitTest : public JsonRpcServiceUnitTest {
 public:
  ...
  GURL gateway_url() { return GURL("https://gateway.wootz.com/"); }
...
class SnsJsonRpcServiceUnitTest : public JsonRpcServiceUnitTest {
 public:
  ...
  SolanaAddress GetMintAddress() const {
    return *wootz_wallet::GetMintAddress(GetDomainKeyAddress());
  }
  ...
  GURL UrlValueV1() const { return GURL("https://v1.wootz.com"); }
  GURL IpfsValueV1() const {
    return GURL(
        "ipfs://v1fybeibd4ala53bs26dvygofvr6ahpa7gbw4eyaibvrbivf4l5rr44yqu4");
  }

  GURL UrlValueV2() const { return GURL("https://v2.wootz.com"); }
...
TEST_F(JsonRpcServiceUnitTest, AnkrGetAccountBalances) {
  ...
  feature_list.InitAndEnableFeature(features::kWootzWalletAnkrBalancesFeature);
...
TEST_F(JsonRpcServiceUnitTest, GetNftMetadatas) {
  ...
  eth_metadata1->image = "https://simplehash.wallet-cdn.wootz.com/assets/1.png";
  ...
  eth_metadata2->image = "https://simplehash.wallet-cdn.wootz.com/assets/2.png";
  ...
  sol_metadata1->image =
      "https://simplehash.wallet-cdn.wootz.com/assets/"
