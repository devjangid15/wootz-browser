...
TEST(WootzWalletUtilsUnitTest, Mnemonic) {
  const struct {
    const char* entropy;
    const char* mnemonic;
    const char* seed;
...
TEST(WootzWalletUtilsUnitTest, MnemonicToSeedAndEntropy) {
...
TEST(WootzWalletUtilsUnitTest, IsValidMnemonic) {
  EXPECT_TRUE(
...
TEST(WootzWalletUtilsUnitTest, EncodeString) {
  std::string output;
...
TEST(WootzWalletUtilsUnitTest, EncodeStringArray) {
...
TEST(WootzWalletUtilsUnitTest, DecodeString) {
  std::string output;
...
TEST(WootzWalletUtilsUnitTest, TransactionReceiptAndValue) {
...
TEST(WootzWalletUtilsTest, IsEndpointUsingWootzWalletProxy) {
  // Test with valid URLs that should match the proxy domains
  EXPECT_TRUE(IsEndpointUsingWootzWalletProxy(
      GURL("https://ethereum-mainnet.wallet.wootz.com")));
  EXPECT_TRUE(IsEndpointUsingWootzWalletProxy(
      GURL("https://ethereum-mainnet.wallet.wootzsoftware.com")));
  EXPECT_TRUE(IsEndpointUsingWootzWalletProxy(
      GURL("https://ethereum-mainnet.wallet.s.wootz.io")));

  // Test with invalid URLs that should not match the proxy domains
  EXPECT_FALSE(IsEndpointUsingWootzWalletProxy(GURL("https://example.com")));
  EXPECT_FALSE(
      IsEndpointUsingWootzWalletProxy(GURL("https://wallet.wootz.io")));
  EXPECT_FALSE(IsEndpointUsingWootzWalletProxy(GURL("https://chrome.com")));
}

TEST(WootzWalletUtilsUnitTest, GetPrefKeyForCoinType) {
...
TEST(WootzWalletUtilsUnitTest, eTLDPlusOne) {
  EXPECT_EQ("", eTLDPlusOne(url::Origin()));
  EXPECT_EQ("wootz.com",
            eTLDPlusOne(url::Origin::Create(GURL("https://blog.wootz.com"))));
  EXPECT_EQ("wootz.com",
            eTLDPlusOne(url::Origin::Create(GURL("https://...wootz.com"))));
  EXPECT_EQ(
      "wootz.com",
      eTLDPlusOne(url::Origin::Create(GURL("https://a.b.c.d.wootz.com/1"))));
  EXPECT_EQ("wootz.github.io", eTLDPlusOne(url::Origin::Create(GURL(
                                   "https://a.b.wootz.github.io/example"))));
  EXPECT_EQ("", eTLDPlusOne(url::Origin::Create(GURL("https://github.io"))));
}

TEST(WootzWalletUtilsUnitTest, MakeOriginInfo) {
  auto origin_info =
      MakeOriginInfo(url::Origin::Create(GURL("https://blog.wootz.com:443")));
  EXPECT_EQ("https://blog.wootz.com", origin_info->origin_spec);
  EXPECT_EQ("wootz.com", origin_info->e_tld_plus_one);
...
TEST(WootzWalletUtilsUnitTest, GenerateRandomHexString) {
...
TEST(WootzWalletUtilsUnitTest, BitcoinNativeAssets) {
  EXPECT_EQ(
...
TEST(WootzWalletUtilsUnitTest, ZcashNativeAssets) {
...
TEST(WootzWalletUtilsUnitTest, GetAllUserAssets) {
...
TEST(WootzWalletUtilsUnitTest, GetUserAsset) {
  sync_preferences::TestingPrefServiceSyncable prefs;
  RegisterProfilePrefs(prefs.registry());
...
TEST(WootzWalletUtilsUnitTest, AddUserAsset) {
  sync_preferences::TestingPrefServiceSyncable prefs;
  RegisterProfilePrefs(prefs.registry());
...
TEST(WootzWalletUtilsUnitTest, EnsureNativeTokenForNetwork) {
  sync_preferences::TestingPrefServiceSyncable prefs;
  RegisterProfilePrefs(prefs.registry());
...
TEST(WootzWalletUtilsUnitTest, RemoveUserAsset) {
  sync_preferences::TestingPrefServiceSyncable prefs;
  RegisterProfilePrefs(prefs.registry());
...
TEST(WootzWalletUtilsUnitTest, SetUserAssetVisible) {
  sync_preferences::TestingPrefServiceSyncable prefs;
  RegisterProfilePrefs(prefs.registry());
...
TEST(WootzWalletUtilsUnitTest, SetAssetSpamStatus) {
  sync_preferences::TestingPrefServiceSyncable prefs;
  RegisterProfilePrefs(prefs.registry());
...
TEST(WootzWalletUtilsUnitTest, SetAssetSPLTokenProgram) {
  sync_preferences::TestingPrefServiceSyncable prefs;
  RegisterProfilePrefs(prefs.registry());
...
TEST(WootzWalletUtilsUnitTest, SetAssetCompressed) {
  sync_preferences::TestingPrefServiceSyncable prefs;
  RegisterProfilePrefs(prefs.registry());
