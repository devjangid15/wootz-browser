...
#include "components/wootz_shields/content/browser/ad_block_service.h"
...
#include "components/cosmetic_filters/browser/cosmetic_filters_resources.h"
...
constexpr char kPasswordWootz[] = "wootz";
...
class AndroidPageAppearingBrowserTest : public PlatformBrowserTest {
 ...
 protected:
  ...
  raw_ptr<wootz_wallet::AssetRatioService> asset_ratio_service_;
  raw_ptr<wootz_wallet::KeyringService> keyring_service_;
  raw_ptr<wootz_wallet::JsonRpcService> json_rpc_service_;
  raw_ptr<wootz_wallet::WootzWalletService> wallet_service_;
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest, TestWalletPageRoute) {
  const GURL expected_real_url =
      GURL("wootzapp://wallet/crypto/portfolio/assets");
  const GURL expected_virtual_url =
      GURL("wootz://wallet/crypto/portfolio/assets");
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest,
  ...
  const GURL expected_url = GURL("wootz://wallet/crypto/portfolio/assets");
  for (const std::string& scheme : GetWebUISchemes()) {
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest, TestSwapPageAppearing) {
  const GURL expected_url = GURL("wootz://wallet/swap");
  for (const std::string& scheme : GetWebUISchemes()) {
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest, TestSendPageAppearing) {
  const GURL expected_url = GURL("wootz://wallet/send");
  for (const std::string& scheme : GetWebUISchemes()) {
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest,
  ...
  const GURL expected_url = GURL("wootz://wallet/crypto/deposit-funds");
  for (const std::string& scheme : GetWebUISchemes()) {
...
IN_PROC_BROWSER_TEST_F(AndroidPageAppearingBrowserTest, TestBuyPageAppearing) {
  const GURL expected_url = GURL("wootz://wallet/crypto/fund-wallet");
  for (const std::string& scheme : GetWebUISchemes()) {
