...
mojom::SwapTransactionParamsUnionPtr GetCannedJupiterTransactionParams(
  ...
  auto quote = wootz_wallet::mojom::JupiterQuote::New();
  ...
  auto platform_fee = wootz_wallet::mojom::JupiterPlatformFee::New();
  platform_fee->amount = "93326";
  platform_fee->fee_bps = "85";
  quote->platform_fee = std::move(platform_fee);

  auto swap_info_1 = wootz_wallet::mojom::JupiterSwapInfo::New();
  ...
  auto step_1 = wootz_wallet::mojom::JupiterRouteStep::New();
  step_1->percent = "100";
  step_1->swap_info = std::move(swap_info_1);

  auto swap_info_2 = wootz_wallet::mojom::JupiterSwapInfo::New();
  ...
  auto step_2 = wootz_wallet::mojom::JupiterRouteStep::New();
  step_2->percent = "100";
...
TEST_F(SwapServiceUnitTest, GetZeroExQuote) {
  ...
  auto source = wootz_wallet::mojom::ZeroExSource::New();
  source->name = "Uniswap_V2";
  source->proportion = "1";
  expected_zero_ex_quote->sources.push_back(source.Clone());

  auto fees = wootz_wallet::mojom::ZeroExFees::New();
  auto zero_ex_fee = wootz_wallet::mojom::ZeroExFee::New();
...
TEST_F(SwapServiceUnitTest, GetZeroExQuoteURL) {
  const std::map<std::string, std::string> non_rfqt_chain_ids = {
      {mojom::kGoerliChainId, "goerli.api.0x.wallet.wootz.com"},
      {mojom::kBnbSmartChainMainnetChainId, "bsc.api.0x.wallet.wootz.com"},
      {mojom::kAvalancheMainnetChainId, "avalanche.api.0x.wallet.wootz.com"},
      {mojom::kFantomMainnetChainId, "fantom.api.0x.wallet.wootz.com"},
      {mojom::kCeloMainnetChainId, "celo.api.0x.wallet.wootz.com"},
      {mojom::kOptimismMainnetChainId, "optimism.api.0x.wallet.wootz.com"},
      {mojom::kArbitrumMainnetChainId, "arbitrum.api.0x.wallet.wootz.com"},
      {mojom::kBaseMainnetChainId, "base.api.0x.wallet.wootz.com"}};

  const std::map<std::string, std::string> rfqt_chain_ids = {
      {mojom::kMainnetChainId, "api.0x.wallet.wootz.com"},
      {mojom::kPolygonMainnetChainId, "polygon.api.0x.wallet.wootz.com"}};
...
TEST_F(SwapServiceUnitTest, GetJupiterQuoteURL) {
  ...
  EXPECT_EQ(url,
            "https://jupiter.wallet.wootz.com/v6/quote?"
...
TEST_F(SwapServiceUnitTest, GetJupiterTransactionURL) {
  ...
  EXPECT_EQ(url, "https://jupiter.wallet.wootz.com/v6/swap");
}
...
TEST_F(SwapServiceUnitTest, GetLiFiQuoteURL) {
  ...
  EXPECT_EQ(url, "https://lifi.wallet.wootz.com/v1/advanced/routes");
}

TEST_F(SwapServiceUnitTest, GetLiFiTransactionURL) {
  auto url = swap_service_->GetLiFiTransactionURL();
  EXPECT_EQ(url, "https://lifi.wallet.wootz.com/v1/advanced/stepTransaction");
}
