...
#include "components/wootz_wallet/browser/bitcoin/bitcoin_rpc.h"
...
#include "components/wootz_wallet/browser/bitcoin_rpc_responses.h"
...
bool ShouldThrottleEndpoint(const GURL& endpoint_host) {
  // Don't throttle requests if host matches wootz proxy.
  return !wootz_wallet::IsEndpointUsingWootzWalletProxy(endpoint_host);
}
...
namespace wootz_wallet::bitcoin_rpc {
