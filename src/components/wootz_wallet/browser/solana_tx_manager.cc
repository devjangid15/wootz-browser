...
#include "components/wootz_wallet/browser/solana_tx_manager.h"
...
#include "components/wootz_wallet/browser/solana_block_tracker.h"
...
#include "components/wootz_wallet/browser/solana_tx_state_manager.h"
...
void SolanaTxManager::RetryTransaction(const std::string& tx_meta_id,
  ...
  if (!meta->IsRetriable()) {
    std::move(callback).Run(
        false, "",
        l10n_util::GetStringUTF8(IDS_WOOTZ_WALLET_TRANSACTION_NOT_RETRIABLE));
    return;
  }
