...
#ifndef COMPONENTS_WOOTZ_WALLET_BROWSER_BLOCKCHAIN_REGISTRY_H_
#define COMPONENTS_WOOTZ_WALLET_BROWSER_BLOCKCHAIN_REGISTRY_H_
...
class BlockchainRegistry : public mojom::BlockchainRegistry {
 ...
 private:
  ...
  std::vector<mojom::BlockchainTokenPtr>* GetTokenListFromChainId(
      const std::string& chain_id);
  std::vector<wootz_wallet::mojom::BlockchainTokenPtr> GetBuyTokens(
      const std::vector<mojom::OnRampProvider>& providers,
      const std::string& chain_id);
...
#endif  // COMPONENTS_WOOTZ_WALLET_BROWSER_BLOCKCHAIN_REGISTRY_H_
