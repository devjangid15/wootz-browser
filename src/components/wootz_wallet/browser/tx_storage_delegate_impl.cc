...
constexpr char kValueStoreDatabaseUMAClientName[] = "WootzWallet";
constexpr base::FilePath::CharType kWalletStorageName[] =
    FILE_PATH_LITERAL("Wootz Wallet Storage");
...
void TxStorageDelegateImpl::RunDBMigrations() {
  ...
  // Added 01/2024
  if (!prefs_->GetBoolean(kWootzWalletTransactionsDBFormatMigrated)) {
    prefs_->SetBoolean(kWootzWalletTransactionsDBFormatMigrated, true);
...
bool TxStorageDelegateImpl::MigrateTransactionsFromPrefsToDB() {
  if (prefs_->GetBoolean(kWootzWalletTransactionsFromPrefsToDBMigrated)) {
    return false;
  }

  if (!prefs_->HasPrefPath(kWootzWalletTransactions)) {
    prefs_->SetBoolean(kWootzWalletTransactionsFromPrefsToDBMigrated, true);
    return false;
  }

  auto& txs = prefs_->GetDict(kWootzWalletTransactions);
  store_->Set(kStorageTransactionsKey, base::Value(txs.Clone()));

  // Keep kWootzWalletTransactions in case we need to revert the migration and
