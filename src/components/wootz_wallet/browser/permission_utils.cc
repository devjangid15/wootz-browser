...
void ExtractAddresses(permissions::RequestType type,
  ...
  std::string match;
  re2::RE2* regex;
  if (type == permissions::RequestType::kWootzEthereum) {
...
bool ParseRequestingOriginInternal(permissions::RequestType type,
                                   ...
                                   std::string* account,
                                   std::queue<std::string>* address_queue) {
  if (origin.opaque() || (type != permissions::RequestType::kWootzEthereum &&
                          type != permissions::RequestType::kWootzSolana)) {
    return false;
  }
...
std::optional<blink::PermissionType> CoinTypeToPermissionType(
  ...
  switch (coin_type) {
    case mojom::CoinType::ETH:
      return blink::PermissionType::WOOTZ_ETHEREUM;
    case mojom::CoinType::SOL:
      return blink::PermissionType::WOOTZ_SOLANA;
    default:
      return std::nullopt;
  }
}
...
std::optional<permissions::RequestType> CoinTypeToPermissionRequestType(
  ...
  switch (coin_type) {
    case mojom::CoinType::ETH:
      return permissions::RequestType::kWootzEthereum;
    case mojom::CoinType::SOL:
      return permissions::RequestType::kWootzSolana;
    default:
      return std::nullopt;
  }
}
