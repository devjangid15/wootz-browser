...
TestWootzWalletServiceDelegate::TestWootzWalletServiceDelegate() {
  EXPECT_TRUE(temp_dir_.CreateUniqueTempDir());
}

base::FilePath TestWootzWalletServiceDelegate::GetWalletBaseDirectory() {
  return temp_dir_.GetPath();
}

bool TestWootzWalletServiceDelegate::IsPrivateWindow() {
  return false;
}

// static
std::unique_ptr<WootzWalletServiceDelegate>
TestWootzWalletServiceDelegate::Create() {
  return std::make_unique<TestWootzWalletServiceDelegate>();
}
