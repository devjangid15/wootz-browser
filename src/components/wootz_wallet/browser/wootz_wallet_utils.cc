...
std::optional<std::string> GetUserAssetAddress(const std::string& address,
  ...
  if (coin == mojom::CoinType::SOL) {
    ...
    if (!::wootz_wallet::IsBase58EncodedSolanaPubkey(address)) {
      return std::nullopt;
    }
    return address;
  }
...
bool ShouldCheckTokenId(const wootz_wallet::mojom::BlockchainTokenPtr& token) {
  return token->is_erc721 || token->is_erc1155;
}

bool TokenMatchesDict(const wootz_wallet::mojom::BlockchainTokenPtr& token,
...
void UpdateLastUnlockPref(PrefService* prefs) {
  prefs->SetTime(kWootzWalletLastUnlockTime, base::Time::Now());
}

bool HasCreatedWallets(PrefService* prefs) {
  return !prefs->GetTime(kWootzWalletLastUnlockTime).is_null();
}
...
mojom::DefaultWallet GetDefaultEthereumWallet(PrefService* prefs) {
  return static_cast<wootz_wallet::mojom::DefaultWallet>(
...
std::vector<mojom::BlockchainTokenPtr> GetAllUserAssets(PrefService* prefs) {
  ...
  const auto& user_assets_list = prefs->GetList(kWootzWalletUserAssetsList);
...
mojom::BlockchainTokenPtr AddUserAsset(PrefService* prefs,
  ...
  ScopedListPrefUpdate update(prefs, kWootzWalletUserAssetsList);
...
mojom::BlowfishOptInStatus GetTransactionSimulationOptInStatus(
  ...
  return static_cast<mojom::BlowfishOptInStatus>(
      prefs->GetInteger(kWootzWalletTransactionSimulationOptInStatus));
}

void SetTransactionSimulationOptInStatus(
    PrefService* prefs,
    const mojom::BlowfishOptInStatus& status) {
  prefs->SetInteger(kWootzWalletTransactionSimulationOptInStatus,
