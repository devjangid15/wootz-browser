...
const char kLegacyWootzWalletDeprecated[] = "legacy_wootz_wallet";
...
void MaybeMigrateSelectedAccountPrefs(
  ...
  if (!profile_prefs->HasPrefPath(kWootzWalletSelectedCoinDeprecated)) {
    return;
  }
  ...
  auto find_account = [&](mojom::KeyringId keyring_id) -> mojom::AccountIdPtr {
    if (!profile_prefs->GetDict(kWootzWalletKeyrings)
  ...
  auto coin = static_cast<mojom::CoinType>(
      profile_prefs->GetInteger(kWootzWalletSelectedCoinDeprecated));
  switch (coin) {
  ...
  profile_prefs->ClearPref(kWootzWalletSelectedCoinDeprecated);
}
...
void MaybeMigratePBKDF2Iterations(PrefService* profile_prefs,
                                  const std::string& password) {
  if (profile_prefs->GetBoolean(kWootzWalletKeyringEncryptionKeysMigrated)) {
    return;
  }

  // Pref is supposed to be set only as true.
  DCHECK(
      !profile_prefs->HasPrefPath(kWootzWalletKeyringEncryptionKeysMigrated));
  ...
  for (auto keyring_id :
    ...
    if (keyring_id == mojom::kDefaultKeyringId) {
      profile_prefs->SetBoolean(kWootzWalletKeyringEncryptionKeysMigrated,
                                true);
    }
...
void MaybeMigrateToWalletMnemonic(PrefService* profile_prefs,
  ...
  if (auto* value =
          GetPrefForKeyring(profile_prefs, kLegacyWootzWalletDeprecated,
                            mojom::KeyringId::kDefault)) {
    if (value->GetIfBool().value_or(false)) {
      profile_prefs->SetBoolean(kWootzWalletLegacyEthSeedFormat, true);
    }
  }

  if (auto* value = GetPrefForKeyring(profile_prefs, kBackupCompleteDeprecated,
                                      mojom::KeyringId::kDefault)) {
    profile_prefs->SetBoolean(kWootzWalletMnemonicBackedUp,
                              value->GetIfBool().value_or(false));
  }

  profile_prefs->SetString(kWootzWalletEncryptorSalt,
  ...
  for (auto keyring_id :
    ...
    SetPrefForKeyring(profile_prefs, kPasswordEncryptorSaltDeprecated,
                      base::Value(), keyring_id);
    SetPrefForKeyring(profile_prefs, kLegacyWootzWalletDeprecated,
                      base::Value(), keyring_id);
