...
class MockWootzWalletServiceDelegateImpl
    : public TestWootzWalletServiceDelegate {
 public:
  MockWootzWalletServiceDelegateImpl() = default;
  ~MockWootzWalletServiceDelegateImpl() override = default;
...
class WalletDataFilesInstallerUnitTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    wootz_wallet_service_ = std::make_unique<WootzWalletService>(
        shared_url_loader_factory_,
        std::make_unique<MockWootzWalletServiceDelegateImpl>(), &prefs_,
        &local_state_);
  ...
  void ImportFromExternalWallet() {
    base::RunLoop run_loop;
    wootz_wallet_service_->ImportFromExternalWallet(
 ...
 private:
  ...
  wootz_component_updater::MockOnDemandUpdater on_demand_updater_;
...
TEST_F(WalletDataFilesInstallerUnitTest,
  ...
  local_state()->SetTime(kWootzWalletLastUnlockTime, base::Time::Now());
  installer().MaybeRegisterWalletDataFilesComponent(updater(), local_state());
  RunUntilIdle();
}
