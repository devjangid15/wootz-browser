...
#include "components/wootz_wallet/browser/swap_request_helper.h"
#include "components/wootz_wallet/browser/swap_response_parser.h"
...
base::flat_map<std::string, std::string> GetHeaders() {
  return {{kWootzServicesKeyHeader, BUILDFLAG(WOOTZ_SERVICES_KEY)}};
}

std::string GetBaseSwapURL(const std::string& chain_id) {
  if (chain_id == wootz_wallet::mojom::kGoerliChainId) {
    return wootz_wallet::kZeroExGoerliBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kMainnetChainId) {
    return wootz_wallet::kZeroExEthereumBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kPolygonMainnetChainId) {
    return wootz_wallet::kZeroExPolygonBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kBnbSmartChainMainnetChainId) {
    return wootz_wallet::kZeroExBinanceSmartChainBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kAvalancheMainnetChainId) {
    return wootz_wallet::kZeroExAvalancheBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kFantomMainnetChainId) {
    return wootz_wallet::kZeroExFantomBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kCeloMainnetChainId) {
    return wootz_wallet::kZeroExCeloBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kOptimismMainnetChainId) {
    return wootz_wallet::kZeroExOptimismBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kArbitrumMainnetChainId) {
    return wootz_wallet::kZeroExArbitrumBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kSolanaMainnet) {
    return wootz_wallet::kJupiterBaseAPIURL;
  } else if (chain_id == wootz_wallet::mojom::kBaseMainnetChainId) {
    return wootz_wallet::kZeroExBaseBaseAPIURL;
  }
...
void SwapService::GetQuote(mojom::SwapQuoteParamsPtr params,
  ...
  std::move(callback).Run(
      nullptr, nullptr, nullptr,
      l10n_util::GetStringUTF8(IDS_WOOTZ_WALLET_UNSUPPORTED_NETWORK));
}
  ...
  if (auto quote = lifi::ParseQuoteResponse(api_request_result.value_body())) {
    if (quote->routes.empty()) {
      ...
      error_response->message =
          l10n_util::GetStringUTF8(IDS_WOOTZ_WALLET_NO_ROUTES_FOUND);
