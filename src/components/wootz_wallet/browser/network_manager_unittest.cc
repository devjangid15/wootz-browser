...
TEST_F(NetworkManagerUnitTest, GetNetworkURLTest) {
  ...
  for (const auto& chain :
    ...
    // Wootz proxies should have infura key added to path.
  ...
  EXPECT_EQ(GURL("https://solana-mainnet.wallet.wootz.com"),
  ...
  EXPECT_EQ(GURL("https://bitcoin-mainnet.wallet.wootz.com/"),
...
TEST_F(NetworkManagerUnitTest, GetNetworkURLForKnownChains) {
  // GetNetworkURL for these known chains should resolve to wootz subdomain.
  base::flat_set<std::string> known_chains = {
      wootz_wallet::mojom::kMainnetChainId,
      wootz_wallet::mojom::kPolygonMainnetChainId,
      wootz_wallet::mojom::kBnbSmartChainMainnetChainId,
      wootz_wallet::mojom::kOptimismMainnetChainId,
      wootz_wallet::mojom::kAuroraMainnetChainId,
      wootz_wallet::mojom::kAvalancheMainnetChainId,
      wootz_wallet::mojom::kGoerliChainId,
      wootz_wallet::mojom::kSepoliaChainId};

  for (const auto& chain :
       NetworkManager::GetAllKnownChains(mojom::CoinType::ETH)) {
    auto network_url =
        network_manager()->GetNetworkURL(chain->chain_id, mojom::CoinType::ETH);
    EXPECT_EQ(base::EndsWith(network_url.host(), ".wootz.com"),
...
TEST_F(NetworkManagerUnitTest, GetSolanaSubdomainForKnownChainId) {
  for (const auto& chain :
    ...
    bool expected = (chain->chain_id == wootz_wallet::mojom::kLocalhostChainId);
...
TEST_F(NetworkManagerUnitTest, GetKnownChain) {
  const base::flat_set<std::string> non_eip1559_networks = {
      wootz_wallet::mojom::kLocalhostChainId,
      wootz_wallet::mojom::kBnbSmartChainMainnetChainId,
      wootz_wallet::mojom::kAuroraMainnetChainId,
      wootz_wallet::mojom::kNeonEVMMainnetChainId};
...
TEST_F(NetworkManagerUnitTest, GetChain) {
  ...
  mojom::NetworkInfo sol_mainnet(
      wootz_wallet::mojom::kSolanaMainnet, "Solana Mainnet Beta",
      {"https://explorer.solana.com/"}, {}, 0,
      {GURL("https://solana-mainnet.wallet.wootz.com")}, "SOL", "Solana", 9,
      wootz_wallet::mojom::CoinType::SOL, {mojom::KeyringId::kSolana});
  ...
  mojom::NetworkInfo fil_mainnet(
      wootz_wallet::mojom::kFilecoinMainnet, "Filecoin Mainnet",
      {"https://filscan.io/tipset/message-detail"}, {}, 0,
      {GURL("https://api.node.glif.io/rpc/v0")}, "FIL", "Filecoin", 18,
      wootz_wallet::mojom::CoinType::FIL, {mojom::KeyringId::kFilecoin});
  ...
  mojom::NetworkInfo btc_mainnet(
      ...
      {GURL("https://bitcoin-mainnet.wallet.wootz.com/")}, "BTC", "Bitcoin", 8,
      mojom::CoinType::BTC,
...
TEST_F(NetworkManagerUnitTest, Eip1559Chain) {
  auto dict = [&] {
    return prefs()->GetDict(kWootzWalletEip1559CustomChains).Clone();
  };
...
TEST_F(NetworkManagerUnitTest, CustomNetworkMatchesKnownNetwork) {
  ...
  EXPECT_EQ(
      network_manager()
          ...
          .GetWithoutFilename(),
      GURL("https://polygon-mainnet.wallet.wootz.com"));
...
TEST_F(NetworkManagerUnitTest, GetAndSetCurrentChainId) {
  ...
  for (const auto coin_type : kAllCoins) {
    ...
    EXPECT_EQ(network_manager()->GetCurrentChainId(coin_type, std::nullopt),
              default_chain_ids.at(coin_type));
    EXPECT_TRUE(network_manager()->SetCurrentChainId(
        coin_type, url::Origin::Create(GURL("wootzapp://wallet")),
        new_default_chain_ids.at(coin_type)));
    EXPECT_EQ(network_manager()->GetCurrentChainId(coin_type, std::nullopt),
              new_default_chain_ids.at(coin_type));
  }
...
TEST_F(NetworkManagerUnitTest, GetUnstoppableDomainsRpcUrl) {
  ...
  EXPECT_EQ(GURL("https://polygon-mainnet.wallet.wootz.com"),
            NetworkManager::GetUnstoppableDomainsRpcUrl(
                mojom::kPolygonMainnetChainId));
}

TEST_F(NetworkManagerUnitTest, GetEnsRpcUrl) {
  EXPECT_EQ(GURL("https://ethereum-mainnet.wallet.wootz.com"),
