...
#include "components/wootz_wallet/browser/zcash/zcash_discover_next_unused_zcash_address_task.h"
#include "components/wootz_wallet/browser/zcash/zcash_get_transparent_utxos_context.h"
...
#include "components/wootz_wallet/browser/zcash/zcash_create_shield_all_transaction_task.h"
  ...
  if (IsUnifiedAddress(address_to)) {
    ...
    if (!transparent) {
      std::move(callback).Run(base::unexpected(l10n_util::GetStringUTF8(
          IDS_WOOTZ_WALLET_ZCASH_UNIFIED_ADDRESS_ERROR)));
      return;
    }
