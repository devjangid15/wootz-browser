...
#include "components/wootz_wallet/browser/solana_tx_meta.h"
...
class SimulationServiceUnitTest : public testing::Test {
 ...
 protected:
  ...
  base::test::ScopedFeatureList feature_list_{
      features::kWootzWalletTransactionSimulationsFeature};
...
TEST_F(SimulationServiceUnitTest, GetScanTransactionURL) {
  ...
  EXPECT_EQ(url,
            "https://blowfish.wallet.wootz.com/ethereum/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kSepoliaChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.wootz.com/ethereum/v0/sepolia/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kPolygonMainnetChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.wootz.com/polygon/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kBnbSmartChainMainnetChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.wootz.com/bnb/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kArbitrumMainnetChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.wootz.com/arbitrum/v0/one/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kBaseMainnetChainId, mojom::CoinType::ETH, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.wootz.com/base/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kSolanaMainnet, mojom::CoinType::SOL, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.wootz.com/solana/v0/mainnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kSolanaDevnet, mojom::CoinType::SOL, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.wootz.com/solana/v0/devnet/scan/"
            "transactions?language=en-US");

  url = simulation_service_->GetScanTransactionURL(
      mojom::kSolanaTestnet, mojom::CoinType::SOL, "en-US");
  EXPECT_EQ(url,
            "https://blowfish.wallet.wootz.com/solana/v0/testnet/scan/"
            "transactions?language=en-US");
}
