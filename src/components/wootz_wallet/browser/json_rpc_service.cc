...
#include "components/wootz_wallet/browser/eth_data_builder.h"
#include "components/wootz_wallet/browser/eth_requests.h"
#include "components/wootz_wallet/browser/eth_response_parser.h"
#include "components/wootz_wallet/browser/fil_requests.h"
#include "components/wootz_wallet/browser/fil_response_parser.h"
#include "components/wootz_wallet/browser/json_rpc_requests_helper.h"
...
#include "components/wootz_wallet/browser/nft_metadata_fetcher.h"
#include "components/wootz_wallet/browser/pref_names.h"
#include "components/wootz_wallet/browser/solana_keyring.h"
#include "components/wootz_wallet/browser/solana_requests.h"
#include "components/wootz_wallet/browser/solana_response_parser.h"
#include "components/wootz_wallet/browser/unstoppable_domains_dns_resolve.h"
#include "components/wootz_wallet/browser/unstoppable_domains_multichain_calls.h"
...
#include "chrome/net/base/url_util.h"
...
using wootz_wallet::mojom::ResolveMethod;
  ...
  return net::DefineNetworkTrafficAnnotation("json_rpc_service", R"(
      semantics {
        ...
        description:
          ...
          "on behalf of the user interacting with the native Wootz wallet."
        trigger:
...
net::NetworkTrafficAnnotationTag GetENSOffchainNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("json_rpc_service", R"(
      ...
      policy {
        cookies_allowed: NO
        setting:
          "You can enable or disable ENS on wootz://settings/extensions page."
        policy_exception_justification:
          "Not implemented."
      }
    )");
...
wootz_wallet::mojom::ResolveMethod ToMojomResolveMethod(
...
decentralized_dns::ResolveMethodTypes FromMojomResolveMethod(
    wootz_wallet::mojom::ResolveMethod method) {
  switch (method) {
...
wootz_wallet::mojom::ResolveMethod ToMojomEnsOffchainResolveMethod(
...
T* GetCustomEVMNetworkFromPrefsDict(const std::string& chain_id,
  ...
  auto* custom_evm_networks =
      custom_networks.FindList(wootz_wallet::kEthereumPrefKey);
  ...
  if (coin == mojom::CoinType::ETH) {
    for (auto& nft_identifier : nft_identifiers) {
      auto checksum_address =
          wootz_wallet::EthAddress::ToEip1191ChecksumAddress(
...
void JsonRpcService::MaybeUpdateIsEip1559(const std::string& chain_id) {
  ...
  if (chain_id != wootz_wallet::mojom::kLocalhostChainId &&
      !network_manager_->CustomChainExists(chain_id, mojom::CoinType::ETH)) {
    return;
  }
...
void JsonRpcService::AnkrGetAccountBalances(
  ...
  auto headers = IsEndpointUsingWootzWalletProxy(GURL(kAnkrAdvancedAPIBaseURL))
