...
namespace {
const char kPassword[] = "wootz123!";

// "wootz.wallet.keyrings" pref for 1.43.93 before encryptor's pbkdf2 iterations
...
// "wootz.wallet.keyrings" pref for 1.66.95 before encrypted mnemonic was
...
class KeyringServiceMigrationsLegacyIterationsUnitTest
 ...
 public:
  void SetupLegacyKeyringPrefs() {
    GetPrefs()->SetDict(kWootzWalletKeyrings,
                        ParseJsonDict(kLegacyKeyringsDict143));

    wootz_wallet::MigrateObsoleteProfilePrefs(GetPrefs());
  }

  void ValidatePrefs() {
    EXPECT_EQ(GetPrefs()->GetDict(kWootzWalletKeyrings),
              ParseJsonDict(kUpgradedKeyringsDict143));
    EXPECT_TRUE(GetPrefs()->GetBoolean(kWootzWalletMnemonicBackedUp));
    EXPECT_EQ(GetPrefs()->GetString(kWootzWalletEncryptorSalt),
              "AQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQE=");
    EXPECT_EQ(GetPrefs()->GetDict(kWootzWalletMnemonic), ParseJsonDict(R"(
    {
    ...
    EXPECT_TRUE(
        GetPrefs()->GetBoolean(kWootzWalletKeyringEncryptionKeysMigrated));
  }
...
TEST_F(KeyringServiceMigrationsLegacyIterationsUnitTest, NoMigration) {
  ...
  EXPECT_FALSE(GetPrefs()->GetBoolean(kWootzWalletMnemonicBackedUp));
  EXPECT_TRUE(
      GetPrefs()->GetBoolean(kWootzWalletKeyringEncryptionKeysMigrated));
...
TEST_F(KeyringServiceMigrationsLegacyIterationsUnitTest,
  ...
  EXPECT_EQ(GetPrefs()->GetDict(kWootzWalletMnemonic), base::Value::Dict());
