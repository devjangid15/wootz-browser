...
base::Value::Dict GetMetadata(const mojom::OriginInfoPtr& origin_info) {
  ...
  if (origin_info && origin_info->origin_spec != "wootzapp://wallet" &&
      origin_info->origin_spec != "wootzapp://wallet") {
    metadata_object.Set("origin", origin_info->origin_spec);
  } else {
    // TODO(onyb): We use "https://chrome.com" as the default origin for now.
    //  This is because Blowfish doesn't support "wootzapp://wallet" and
    //  "wootzapp://wallet" as origins yet. We'll update this once Blowfish
    //  supports them.
    metadata_object.Set("origin", "https://chrome.com");
  }
