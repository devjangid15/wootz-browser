...
#include "components/wootz_wallet/browser/asset_ratio_response_parser.h"
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("asset_ratio_service", R"(
      semantics {
        ...
        description:
          "This service is used to obtain asset prices for the Wootz wallet."
        trigger:
...
std::string TimeFrameKeyToString(
    wootz_wallet::mojom::AssetPriceTimeframe timeframe) {
  std::string timeframe_key;
  switch (timeframe) {
    case wootz_wallet::mojom::AssetPriceTimeframe::Live:
      timeframe_key = "live";
      break;
    case wootz_wallet::mojom::AssetPriceTimeframe::OneDay:
      timeframe_key = "1d";
      break;
    case wootz_wallet::mojom::AssetPriceTimeframe::OneWeek:
      timeframe_key = "1w";
      break;
    case wootz_wallet::mojom::AssetPriceTimeframe::OneMonth:
      timeframe_key = "1m";
      break;
    case wootz_wallet::mojom::AssetPriceTimeframe::ThreeMonths:
      timeframe_key = "3m";
      break;
    case wootz_wallet::mojom::AssetPriceTimeframe::OneYear:
      timeframe_key = "1y";
      break;
    case wootz_wallet::mojom::AssetPriceTimeframe::All:
      timeframe_key = "all";
      break;
  }
...
std::optional<std::string> ChainIdToStripeChainId(const std::string& chain_id) {
  static base::NoDestructor<base::flat_map<std::string, std::string>>
      chain_id_lookup({{wootz_wallet::mojom::kMainnetChainId, "ethereum"},
                       {wootz_wallet::mojom::kSolanaMainnet, "solana"},
                       {wootz_wallet::mojom::kPolygonMainnetChainId, "polygon"},
                       {wootz_wallet::mojom::kBitcoinMainnet, "bitcoin"}});
...
void AssetRatioService::GetPrice(const std::vector<std::string>& from_assets,
  ...
  api_request_helper_->Request(
      ...
      std::move(internal_callback), MakeWootzServicesKeyHeaders(),
