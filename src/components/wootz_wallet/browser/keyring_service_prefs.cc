...
const base::Value* GetPrefForKeyring(PrefService* profile_prefs,
  ...
  const auto& keyrings_pref = profile_prefs->GetDict(kWootzWalletKeyrings);
...
bool SetSelectedWalletAccountInPrefs(PrefService* profile_prefs,
  ...
  if (unique_key ==
      profile_prefs->GetString(kWootzWalletSelectedWalletAccount)) {
    return false;
  }

  profile_prefs->SetString(kWootzWalletSelectedWalletAccount, unique_key);
  return true;
}
