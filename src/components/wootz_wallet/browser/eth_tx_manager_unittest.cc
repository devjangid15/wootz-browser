...
#include "components/wootz_wallet/browser/eth_data_parser.h"
...
class EthTxManagerUnitTest : public testing::Test {
 public:
  ...
  url::Origin GetOrigin() const {
    return url::Origin::Create(GURL("https://chrome.com"));
  }
...
TEST_F(EthTxManagerUnitTest, AddUnapprovedEvmTransaction) {
  ...
  // Known Eip1559 chain.
  {
    ...
    EXPECT_EQ(tx_meta->origin(), url::Origin::Create(GURL("wootzapp://wallet")));
...
TEST_F(EthTxManagerUnitTest, Reset) {
  ...
  GetPrefs()->Set(kWootzWalletTransactions, ParseJson(R"({"ethereum":{}})"));

  tx_service_->Reset();

  EXPECT_FALSE(GetPrefs()->HasPrefPath(kWootzWalletTransactions));
