...
#include "components/wootz_wallet/browser/meld_integration_response_parser.h"
...
#include "components/wootz_wallet/browser/meld_integration_responses.h"
...
wootz_wallet::mojom::MeldLogoImagesPtr ParseMeldLogos(
    const std::optional<wootz_wallet::meld_integration_responses::Logos>&
        logos) {
  if (!logos) {
    return nullptr;
  }

  return wootz_wallet::mojom::MeldLogoImages::New(
      logos->dark, logos->dark_short, logos->light, logos->light_short);
}

std::optional<std::vector<wootz_wallet::mojom::MeldRegionPtr>> ParseMeldRegions(
    const std::optional<
        std::vector<wootz_wallet::meld_integration_responses::Region>>&
        regions) {
  if (!regions) {
    return std::nullopt;
  }

  std::vector<wootz_wallet::mojom::MeldRegionPtr> result;
  for (const auto& region_value : *regions) {
    auto reg = wootz_wallet::mojom::MeldRegion::New(region_value.region_code,
                                                    region_value.name);
