...
const base::flat_map<std::string, std::string> kInfuraSubdomains = {
    {wootz_wallet::mojom::kMainnetChainId, "mainnet"},
    {wootz_wallet::mojom::kGoerliChainId, "goerli"},
    {wootz_wallet::mojom::kSepoliaChainId, "sepolia"}};

// DEPRECATED 01/2024. For migration only.
const base::flat_map<std::string, std::string> kSolanaSubdomains = {
    {wootz_wallet::mojom::kSolanaMainnet, "mainnet"},
    {wootz_wallet::mojom::kSolanaTestnet, "testnet"},
    {wootz_wallet::mojom::kSolanaDevnet, "devnet"}};

// DEPRECATED 01/2024. For migration only.
const base::flat_map<std::string, std::string> kFilecoinSubdomains = {
    {wootz_wallet::mojom::kFilecoinMainnet, "mainnet"},
    {wootz_wallet::mojom::kFilecoinTestnet, "testnet"}};
...
const base::Value::List* GetCustomNetworksList(PrefService* prefs,
                                               mojom::CoinType coin) {
  const auto& custom_networks = prefs->GetDict(kWootzWalletCustomNetworks);
...
std::string GetCurrentChainIdFromPrefs(PrefService* prefs,
                                       mojom::CoinType coin) {
  const auto& selected_networks = prefs->GetDict(kWootzWalletSelectedNetworks);
...
// static
mojom::NetworkInfoPtr NetworkManager::GetKnownChain(const std::string& chain_id,
                                                    mojom::CoinType coin) {
...
mojom::NetworkInfoPtr NetworkManager::GetCustomChain(
  ...
  for (const auto& it : *custom_list) {
    if (auto opt_chain_id =
            wootz_wallet::ExtractChainIdFromValue(it.GetIfDict())) {
      if (base::CompareCaseInsensitiveASCII(chain_id, *opt_chain_id) == 0) {
        return wootz_wallet::ValueToNetworkInfo(it);
      }
    }
  }
  return nullptr;
...
bool NetworkManager::KnownChainExists(const std::string& chain_id,
                                      mojom::CoinType coin) {
  if (coin == mojom::CoinType::ETH) {
    for (const auto* network : GetKnownEthNetworks()) {
      if (base::CompareCaseInsensitiveASCII(network->chain_id, chain_id) == 0) {
...
bool NetworkManager::CustomChainExists(const std::string& custom_chain_id,
                                       mojom::CoinType coin) {
  const base::Value::List* custom_list = GetCustomNetworksList(prefs_, coin);
  if (!custom_list) {
    return false;
...
std::optional<bool> NetworkManager::IsEip1559Chain(
  ...
  if (auto is_eip_1559 = prefs_->GetDict(kWootzWalletEip1559CustomChains)
...
void NetworkManager::SetEip1559ForCustomChain(const std::string& chain_id,
  ...
  auto chain_id_lwr = base::ToLowerASCII(chain_id);
  ScopedDictPrefUpdate update(prefs_, kWootzWalletEip1559CustomChains);
...
void NetworkManager::AddCustomNetwork(const mojom::NetworkInfo& chain) {
  ScopedDictPrefUpdate update(prefs_, kWootzWalletCustomNetworks);
...
std::vector<std::string> NetworkManager::GetHiddenNetworks(
  ...
  const auto& hidden_networks = prefs_->GetDict(kWootzWalletHiddenNetworks);
...
void NetworkManager::AddHiddenNetwork(mojom::CoinType coin,
                                      const std::string& chain_id) {
  ScopedDictPrefUpdate update(prefs_, kWootzWalletHiddenNetworks);
...
bool NetworkManager::SetCurrentChainId(mojom::CoinType coin,
  ...
  if (!origin) {
    ScopedDictPrefUpdate update(prefs_, kWootzWalletSelectedNetworks);
    ...
    if (origin->scheme() == url::kHttpScheme ||
      ...
      ScopedDictPrefUpdate update(prefs_,
                                  kWootzWalletSelectedNetworksPerOrigin);
