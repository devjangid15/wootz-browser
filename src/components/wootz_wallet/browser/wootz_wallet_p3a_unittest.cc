...
#include "components/wootz_wallet/browser/zcash/zcash_tx_manager.h"
...
class WootzWalletP3AUnitTest : public testing::Test {
 public:
  WootzWalletP3AUnitTest()
  ...
  void SetUp() override {
    ...
    wootz_wallet::RegisterLocalStatePrefsForMigration(local_state_.registry());

    bitcoin_test_rpc_server_ = std::make_unique<BitcoinTestRpcServer>();

    wootz_wallet_service_ = std::make_unique<WootzWalletService>(
        shared_url_loader_factory_, TestWootzWalletServiceDelegate::Create(),
        &prefs_, &local_state_);
    wootz_wallet_service_->json_rpc_service()->SetAPIRequestHelperForTesting(
        shared_url_loader_factory_);
    keyring_service_ = wootz_wallet_service_->keyring_service();
    wootz_wallet_service_->GetBitcoinWalletService()
        ->SetUrlLoaderFactoryForTesting(
            bitcoin_test_rpc_server_->GetURLLoaderFactory());
    wootz_wallet_service_->GetZcashWalletService()->SetZCashRpcForTesting(
        std::make_unique<ZCashRpc>(wootz_wallet_service_->network_manager(),
                                   shared_url_loader_factory_));
    tx_service_ = wootz_wallet_service_->tx_service();
    WaitForTxStorageDelegateInitialized(tx_service_->GetDelegateForTesting());
    wallet_p3a_ = wootz_wallet_service_->GetWootzWalletP3A();
  }
 ...
 protected:
  ...
  raw_ptr<WootzWalletP3A> wallet_p3a_;
  scoped_refptr<network::SharedURLLoaderFactory> shared_url_loader_factory_;
  network::TestURLLoaderFactory url_loader_factory_;
  data_decoder::test::InProcessDataDecoder in_process_data_decoder_;
};

TEST_F(WootzWalletP3AUnitTest, KeyringCreated) {
...
TEST_F(WootzWalletP3AUnitTest, ReportOnboardingAction) {
...
TEST_F(WootzWalletP3AUnitTest, ReportOnboardingActionRestore) {
  histogram_tester_->ExpectTotalCount(kOnboardingConversionHistogramName, 0);
...
TEST_F(WootzWalletP3AUnitTest, ReportTransactionSent) {
...
TEST_F(WootzWalletP3AUnitTest, ActiveAccounts) {
...
TEST_F(WootzWalletP3AUnitTest, NewUserBalance) {
  // record first usage
...
TEST_F(WootzWalletP3AUnitTest, NewUserBalancePastDeadline) {
  // record first usage
  wallet_p3a_->ReportUsage(true);
...
TEST_F(WootzWalletP3AUnitTest, JSProviders) {
...
TEST_F(WootzWalletP3AUnitTest, NFTGalleryViews) {
  histogram_tester_->ExpectTotalCount(kWootzWalletNFTCountHistogramName, 0);
  histogram_tester_->ExpectTotalCount(kWootzWalletNFTNewUserHistogramName, 0);

  wallet_p3a_->RecordNFTGalleryView(0);
  histogram_tester_->ExpectUniqueSample(kWootzWalletNFTCountHistogramName, 0,
                                        1);
  histogram_tester_->ExpectUniqueSample(kWootzWalletNFTNewUserHistogramName, 1,
                                        1);

  wallet_p3a_->RecordNFTGalleryView(6);
  histogram_tester_->ExpectBucketCount(kWootzWalletNFTCountHistogramName, 2, 1);
  // new user histogram should only be reported once, ever
  histogram_tester_->ExpectUniqueSample(kWootzWalletNFTNewUserHistogramName, 1,
                                        1);
}

TEST_F(WootzWalletP3AUnitTest, NFTDiscoveryEnabled) {
  histogram_tester_->ExpectTotalCount(
      kWootzWalletNFTDiscoveryEnabledHistogramName, 0);

  local_state_.SetTime(kWootzWalletLastUnlockTime, base::Time::Now());
  histogram_tester_->ExpectUniqueSample(
      kWootzWalletNFTDiscoveryEnabledHistogramName, 0, 1);

  prefs_.SetBoolean(kWootzWalletNftDiscoveryEnabled, true);
  histogram_tester_->ExpectBucketCount(
      kWootzWalletNFTDiscoveryEnabledHistogramName, 1, 1);

  prefs_.SetBoolean(kWootzWalletNftDiscoveryEnabled, false);
  histogram_tester_->ExpectBucketCount(
      kWootzWalletNFTDiscoveryEnabledHistogramName, 0, 2);
}

TEST_F(WootzWalletP3AUnitTest, EthTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kEthTransactionSentHistogramName, 0);
...
TEST_F(WootzWalletP3AUnitTest, TestnetEthTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kEthTransactionSentHistogramName, 0);
...
TEST_F(WootzWalletP3AUnitTest, SolTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kSolTransactionSentHistogramName, 0);
...
TEST_F(WootzWalletP3AUnitTest, FilTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kFilTransactionSentHistogramName, 0);
...
TEST_F(WootzWalletP3AUnitTest, BtcTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kBtcTransactionSentHistogramName, 0);
...
TEST_F(WootzWalletP3AUnitTest, ZecTransactionSentObservation) {
  histogram_tester_->ExpectTotalCount(kZecTransactionSentHistogramName, 0);
