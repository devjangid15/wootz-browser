...
#include "components/wootz_wallet/common/eth_requests.h"
#include "components/wootz_wallet/common/hex_utils.h"
#include "components/wootz_wallet/common/json_rpc_requests.h"
...
std::optional<base::Value::List> GetParamsList(const std::string& json) {
  ...
  auto* params = value.FindListByDottedPath(wootz_wallet::kParams);
  if (!params) {
    return std::nullopt;
  }
...
std::optional<base::Value::Dict> GetParamsDict(const std::string& json) {
  ...
  auto* value = json_value->GetDict().FindDict(wootz_wallet::kParams);
  if (!value) {
    return std::nullopt;
  }
...
wootz_wallet::mojom::TxDataPtr ValueToTxData(
    const wootz_wallet::json_rpc_requests::Transaction& tx,
    std::string* from_out) {
  CHECK(from_out);
  auto tx_data = wootz_wallet::mojom::TxData::New();
  ...
  *from_out = tx.from;
  ...
  if (tx.data && !tx.data->empty() &&
      !wootz_wallet::PrefixedHexStringToBytes(*tx.data, &bytes)) {
    return nullptr;
  }
  tx_data->data = bytes;
...
mojom::TxDataPtr ParseEthTransactionParams(const std::string& json,
  ...
  auto tx = wootz_wallet::json_rpc_requests::Transaction::FromValue(*param_obj);
  if (!tx) {
    return nullptr;
  }
...
bool ShouldCreate1559Tx(wootz_wallet::mojom::TxData1559Ptr tx_data_1559,
