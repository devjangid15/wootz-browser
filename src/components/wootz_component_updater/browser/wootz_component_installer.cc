...
WootzComponentInstallerPolicy::WootzComponentInstallerPolicy(
...
WootzComponentInstallerPolicy::~WootzComponentInstallerPolicy() = default;

bool WootzComponentInstallerPolicy::VerifyInstallation(
    const base::Value::Dict& manifest,
...
bool WootzComponentInstallerPolicy::SupportsGroupPolicyEnabledComponentUpdates()
    const {
  return false;
}

bool WootzComponentInstallerPolicy::RequiresNetworkEncryption() const {
  return false;
}

update_client::CrxInstaller::Result
WootzComponentInstallerPolicy::OnCustomInstall(
    const base::Value::Dict& manifest,
    const base::FilePath& install_dir) {
  return Result(InstallError::NONE);
}

void WootzComponentInstallerPolicy::OnCustomUninstall() {}

void WootzComponentInstallerPolicy::ComponentReady(
...
base::FilePath WootzComponentInstallerPolicy::GetRelativeInstallDir() const {
...
void WootzComponentInstallerPolicy::GetHash(std::vector<uint8_t>* hash) const {
...
std::string WootzComponentInstallerPolicy::GetName() const {
  return name_;
}

update_client::InstallerAttributes
WootzComponentInstallerPolicy::GetInstallerAttributes() const {
  return update_client::InstallerAttributes();
}

bool WootzComponentInstallerPolicy::IsWootzComponent() const {
  return true;
}
...
void RegisterComponent(component_updater::ComponentUpdateService* cus,
  ...
  auto installer = base::MakeRefCounted<component_updater::ComponentInstaller>(
      std::make_unique<WootzComponentInstallerPolicy>(
