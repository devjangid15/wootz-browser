...
#include "components/wootz_adblock/core/browser/ad_block_list_p3a.h"
...
using wootz_adblock::features::kWootzAdblockCookieListDefault;
using wootz_adblock::features::kWootzAdblockExperimentalListDefault;
using wootz_adblock::features::kWootzAdblockMobileNotificationsListDefault;
...
void AdBlockComponentServiceManager::StartRegionalServices() {
  ...
  if (!checked_all_default_regions) {
    ...
    auto locale_lists = wootz_adblock::FindAdBlockFilterListsByLocale(
...
bool AdBlockComponentServiceManager::IsFilterListAvailable(
  ...
  DCHECK(!uuid.empty());
  auto catalog_entry =
      wootz_adblock::FindAdBlockFilterListByUUID(filter_list_catalog_, uuid);
...
void AdBlockComponentServiceManager::UpdateFilterLists(
  ...
  wootz_component_updater::WootzOnDemandUpdater::GetInstance()->OnDemandUpdate(
