...
#include "components/wootz_adblock/content/browser/ad_block_subscription_download_manager.h"
...
const net::NetworkTrafficAnnotationTag
    kWootzShieldsAdBlockSubscriptionTrafficAnnotation =
        net::DefineNetworkTrafficAnnotation(
            "wootz_adblock_ad_block_subscription",
            R"(
        semantics {
          sender: "Wootz Shields"
          description:
            "Wootz periodically downloads updates to third-party filter lists "
            "added by users on wootz://adblock."
          trigger:
            "After being registered in wootz://adblock, any enabled filter "
            "list subscriptions will be updated in accordance with their "
            "`Expires` field if present, or daily otherwise. A manual refresh "
            "for a particular list can also be triggered in wootz://adblock."
          data: "The URL endpoint provided by the user in wootz://adblock to "
            "fetch list updates from. No user information is sent."
          destination: WOOTZ_OWNED_SERVICE
        }
        policy {
          cookies_allowed: NO
          setting:
            ...
            "wootz://adblock page's custom list subscription section."
...
void AdBlockSubscriptionDownloadManager::OnDownloadFailed(
  ...
  base::UmaHistogramBoolean(
      "WootzShields.AdBlockSubscriptionDownloadManager.DownloadSucceeded",
      false);
