...
#include "components/wootz_adblock/content/browser/wootz_adblock_p3a.h"
#include "components/wootz_adblock/core/common/wootz_adblock_utils.h"
...
#include "components/content_settings/core/common/content_settings_util.h"
#include "components/https_upgrade_exceptions/browser/https_upgrade_exceptions_service.h"
...
void RecordShieldsToggled(PrefService* local_state) {
  ::wootz_adblock::MaybeRecordShieldsUsageP3A(::wootz_adblock::kShutOffShields,
                                              local_state);
}

void RecordShieldsSettingChanged(PrefService* local_state) {
  ::wootz_adblock::MaybeRecordShieldsUsageP3A(
      ::wootz_adblock::kChangedPerSiteShields, local_state);
}
...
class WootzCookieRules {
 public:
  WootzCookieRules(ContentSetting general_setting,
  ...
  static WootzCookieRules GetDefault(
  ...
  void Merge(const WootzCookieRules& other) {
...
base::Value::Dict GetShieldsMetadata(HostContentSettingsMap* map,
  ...
  auto shields_metadata_value = map->GetWebsiteSetting(
      url, url, ContentSettingsType::wootz_adblock_METADATA);
...
void SetWootzAdblockEnabled(HostContentSettingsMap* map,
                            bool enable,
                            const GURL& url,
  ...
  map->SetContentSettingCustomScope(
      ...
      ContentSettingsType::wootz_adblock,
      // this is 'allow_wootz_adblock' so 'enable' == 'allow'
      enable ? CONTENT_SETTING_ALLOW : CONTENT_SETTING_BLOCK);

  RecordShieldsToggled(local_state);
}

void ResetWootzAdblockEnabled(HostContentSettingsMap* map, const GURL& url) {
  if (url.is_valid() && !url.SchemeIsHTTPOrHTTPS()) {
    return;
  }
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::wootz_adblock, CONTENT_SETTING_DEFAULT);
}

bool GetWootzAdblockEnabled(HostContentSettingsMap* map, const GURL& url) {
  if (base::FeatureList::IsEnabled(
          ::wootz_adblock::features::kWootzExtensionNetworkBlocking) &&
  ...
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::wootz_adblock);

  // see EnableWootzAdblock - allow and default == true
...
void SetAdControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::WOOTZ_ADS, GetDefaultBlockFromControlType(type));

  map->SetContentSettingCustomScope(primary_pattern,
                                    ContentSettingsPattern::Wildcard(),
                                    ContentSettingsType::WOOTZ_TRACKERS,
...
ControlType GetAdControlType(HostContentSettingsMap* map, const GURL& url) {
  ...
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::WOOTZ_ADS);
...
void SetCosmeticFilteringControlType(HostContentSettingsMap* map,
  ...
  base::Value web_setting = map->GetWebsiteSetting(
      url, GURL(), ContentSettingsType::WOOTZ_COSMETIC_FILTERING,
      &setting_info);
  bool was_default =
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::WOOTZ_COSMETIC_FILTERING,
      GetDefaultBlockFromControlType(type));
...
bool IsReduceLanguageEnabledForProfile(PrefService* pref_service) {
  ...
  if (!base::FeatureList::IsEnabled(features::kWootzReduceLanguage)) {
    return false;
  }

  // Don't reduce language if user preference is unchecked
  if (!pref_service->GetBoolean(wootz_adblock::prefs::kReduceLanguageEnabled)) {
    return false;
  }
...
bool ShouldDoReduceLanguage(HostContentSettingsMap* map,
  ...
  // Don't reduce language if Wootz Adblock is down (this also handles cases
  // where the URL is not HTTP(S))
  if (!wootz_adblock::GetWootzAdblockEnabled(map, url)) {
    return false;
  }

  // Don't reduce language if fingerprinting is off
  if (wootz_adblock::GetFingerprintingControlType(map, url) ==
      ControlType::ALLOW) {
    return false;
  }

  // Don't reduce language if there's a webcompat exception
  if (wootz_adblock::IsWebcompatEnabled(
          map, ContentSettingsType::WOOTZ_WEBCOMPAT_LANGUAGE, url)) {
    return false;
  }
...
DomainBlockingType GetDomainBlockingType(HostContentSettingsMap* map,
  ...
  if (!base::FeatureList::IsEnabled(
          wootz_adblock::features::kWootzDomainBlock)) {
    return DomainBlockingType::kNone;
  }

  // Don't block if Wootz Adblock is down (this also handles cases where
  ...
  if (wootz_adblock::GetAdControlType(map, url) != ControlType::BLOCK) {
    return DomainBlockingType::kNone;
  }

  const ControlType cosmetic_control_type =
      wootz_adblock::GetCosmeticFilteringControlType(map, url);
  ...
  if (cosmetic_control_type == BLOCK_THIRD_PARTY &&
      ...
      base::FeatureList::IsEnabled(
          wootz_adblock::features::kWootzDomainBlock1PES)) {
...
void SetCookieControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(patterns.host_pattern,
                                    ContentSettingsPattern::Wildcard(),
                                    ContentSettingsType::WOOTZ_REFERRERS,
                                    GetDefaultBlockFromControlType(type));
  ...
  switch (type) {
    case ControlType::BLOCK_THIRD_PARTY:
      ...
      map->SetContentSettingCustomScope(
          ...
          ContentSettingsType::WOOTZ_COOKIES, CONTENT_SETTING_BLOCK);
      // first-party rule:
      map->SetContentSettingCustomScope(
          patterns.domain_pattern, patterns.host_pattern,
          ContentSettingsType::WOOTZ_COOKIES, CONTENT_SETTING_ALLOW);
      break;
    case ControlType::ALLOW:
    case ControlType::BLOCK:
      ...
      map->SetContentSettingCustomScope(
          patterns.domain_pattern, patterns.host_pattern,
          ContentSettingsType::WOOTZ_COOKIES, CONTENT_SETTING_DEFAULT);
      // general-rule:
      map->SetContentSettingCustomScope(
          ContentSettingsPattern::Wildcard(), patterns.host_pattern,
          ContentSettingsType::WOOTZ_COOKIES,
...
ControlType GetCookieControlType(
  ...
  auto result = WootzCookieRules::Get(map, url);
  if (result.HasDefault()) {
    const auto default_rules = WootzCookieRules::GetDefault(cookie_settings);
...
bool AreReferrersAllowed(HostContentSettingsMap* map, const GURL& url) {
  const ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::WOOTZ_REFERRERS);
...
void SetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  content_settings::SettingInfo setting_info;
  base::Value web_setting = map->GetWebsiteSetting(
      url, GURL(), ContentSettingsType::WOOTZ_FINGERPRINTING_V2, &setting_info);
  bool was_default =
      web_setting.is_none() || setting_info.primary_pattern.MatchesAllHosts() ||
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::WOOTZ_FINGERPRINTING_V2, content_setting);
  if (!map->IsOffTheRecord()) {
    // Only report to P3A if not a guest/incognito profile
    RecordShieldsSettingChanged(local_state);
    if (url.is_empty()) {
...
ControlType GetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  ContentSettingsForOneType fingerprinting_rules =
      map->GetSettingsForOneType(ContentSettingsType::WOOTZ_FINGERPRINTING_V2);

  ContentSetting fp_setting =
      GetWootzFPContentSettingFromRules(fingerprinting_rules, url);
...
bool IsWootzAdblockManaged(PrefService* prefs,
                           HostContentSettingsMap* map,
                           GURL url) {
  DCHECK(prefs);
  DCHECK(map);
  content_settings::SettingInfo info;
  map->GetWebsiteSetting(url, url, ContentSettingsType::wootz_adblock, &info);
  return info.source == content_settings::SettingSource::kPolicy;
}

bool IsHttpsByDefaultFeatureEnabled() {
  return base::FeatureList::IsEnabled(net::features::kWootzHttpsByDefault);
}

bool IsShowStrictFingerprintingModeEnabled() {
  return base::FeatureList::IsEnabled(
      features::kWootzShowStrictFingerprintingMode);
}
...
void SetHttpsUpgradeControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::WOOTZ_HTTPS_UPGRADE, setting);
...
ControlType GetHttpsUpgradeControlType(HostContentSettingsMap* map,
  ...
  ContentSetting setting = map->GetContentSetting(
      url, GURL(), ContentSettingsType::WOOTZ_HTTPS_UPGRADE);
  if (setting == CONTENT_SETTING_ALLOW) {
    // Disabled (allow http)
    return ControlType::ALLOW;
...
bool ShouldUpgradeToHttps(
  ...
  if (!GetWootzAdblockEnabled(map, url)) {
    return false;
  }
...
bool ShouldForceHttps(HostContentSettingsMap* map, const GURL& url) {
  return GetWootzAdblockEnabled(map, url) &&
...
void SetForgetFirstPartyStorageEnabled(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::WOOTZ_REMEMBER_1P_STORAGE,
...
bool GetForgetFirstPartyStorageEnabled(HostContentSettingsMap* map,
                                       const GURL& url) {
  ContentSetting setting = map->GetContentSetting(
      url, url, ContentSettingsType::WOOTZ_REMEMBER_1P_STORAGE);
...
ShieldsSettingCounts GetAdsSettingCount(HostContentSettingsMap* map) {
  ContentSettingsForOneType cosmetic_rules =
      map->GetSettingsForOneType(ContentSettingsType::WOOTZ_COSMETIC_FILTERING);
...
mojom::FarblingLevel GetFarblingLevel(HostContentSettingsMap* map,
  ...
  if (!base::FeatureList::IsEnabled(features::kWootzFarbling)) {
    return wootz_adblock::mojom::FarblingLevel::OFF;
  }

  const bool shields_up = GetWootzAdblockEnabled(map, primary_url);
  if (!shields_up) {
    return wootz_adblock::mojom::FarblingLevel::OFF;
  }
  ...
  switch (fingerprinting_type) {
    case ControlType::ALLOW:
      return wootz_adblock::mojom::FarblingLevel::OFF;
    case ControlType::BLOCK:
      return wootz_adblock::mojom::FarblingLevel::MAXIMUM;
    case ControlType::BLOCK_THIRD_PARTY:
      NOTREACHED();
    case ControlType::DEFAULT:
      return wootz_adblock::mojom::FarblingLevel::BALANCED;
  }
}
