...
#include "chrome/browser/profiles/brave_profile_manager.h"
...
using wootz_adblock::ControlType;
using wootz_adblock::ControlTypeFromString;
using wootz_adblock::ControlTypeToString;
using wootz_adblock::DomainBlockingType;
using wootz_adblock::GetPatternFromURL;
using wootz_adblock::features::kBraveDomainBlock;
...
class BraveShieldsUtilTest : public testing::Test {
 public:
  ...
  void ExpectDomainBlockingType(const GURL& url,
    ...
    auto setting = wootz_adblock::GetDomainBlockingType(map, url);
...
/* wootz_adblock CONTROL */
TEST_F(BraveShieldsUtilTest, SetBraveShieldsEnabled_ForOrigin) {
  auto* map = HostContentSettingsMapFactory::GetForProfile(profile());

  wootz_adblock::SetBraveShieldsEnabled(map, false, GURL("http://brave.com"));
  ...
  wootz_adblock::SetBraveShieldsEnabled(map, false, host1);
  // host2.com is enabled by default
  setting =
      map->GetContentSetting(host2, GURL(), ContentSettingsType::wootz_adblock);
  EXPECT_EQ(CONTENT_SETTING_ALLOW, setting);
  ...
  setting =
      map->GetContentSetting(host2, GURL(), ContentSettingsType::wootz_adblock);
  EXPECT_EQ(CONTENT_SETTING_BLOCK, setting);
  wootz_adblock::SetBraveShieldsEnabled(map, true, host2);

  // setting should not be changed.
  EXPECT_FALSE(wootz_adblock::GetBraveShieldsEnabled(map, host2));

  // setting should apply enabled to origin.
  EXPECT_TRUE(wootz_adblock::GetBraveShieldsEnabled(map, host1));
  wootz_adblock::SetBraveShieldsEnabled(map, false, host1);
  // setting should not be changed.
  EXPECT_TRUE(wootz_adblock::GetBraveShieldsEnabled(map, host1));

  // setting should not apply to default
  EXPECT_TRUE(wootz_adblock::GetBraveShieldsEnabled(map, GURL()));
}
...
TEST_F(BraveShieldsUtilTest, IsBraveShieldsManaged) {
  ...
  GURL host2("http://host2.com");
  GURL host1("http://host1.com");
  EXPECT_FALSE(wootz_adblock::IsBraveShieldsManaged(
  ...
  EXPECT_TRUE(wootz_adblock::IsBraveShieldsManaged(
      profile()->GetTestingPrefService(), map, host2));
...
TEST_F(BraveShieldsUtilTest, SetBraveShieldsEnabled_IsNotHttpHttps) {
  auto* map = HostContentSettingsMapFactory::GetForProfile(profile());
  auto setting =
      wootz_adblock::GetBraveShieldsEnabled(map, GURL("chrome://preferences"));
  EXPECT_EQ(false, setting);
  wootz_adblock::SetBraveShieldsEnabled(map, ControlType::ALLOW,
                                        GURL("chrome://preferences"));
  setting =
      wootz_adblock::GetBraveShieldsEnabled(map, GURL("chrome://preferences"));
  EXPECT_EQ(false, setting);

  setting = wootz_adblock::GetBraveShieldsEnabled(map, GURL("about:blank"));
  EXPECT_EQ(false, setting);
  wootz_adblock::SetBraveShieldsEnabled(map, ControlType::ALLOW,
                                        GURL("about:blank"));
  setting = wootz_adblock::GetBraveShieldsEnabled(map, GURL("about:blank"));
...
TEST_F(BraveShieldsUtilTest, GetBraveShieldsEnabled_ForOrigin) {
  ...
  auto setting = wootz_adblock::GetBraveShieldsEnabled(map, GURL());
  EXPECT_EQ(true, setting);
  setting =
      wootz_adblock::GetBraveShieldsEnabled(map, GURL("http://brave.com"));
  EXPECT_EQ(true, setting);
  setting =
      wootz_adblock::GetBraveShieldsEnabled(map, GURL("https://brave.com"));
  EXPECT_EQ(true, setting);

  /* BLOCK */
  map->SetContentSettingCustomScope(
      ContentSettingsPattern::FromString("http://brave.com/*"),
      ContentSettingsPattern::Wildcard(), ContentSettingsType::wootz_adblock,
  ...
  setting = wootz_adblock::GetBraveShieldsEnabled(map, GURL());
  EXPECT_EQ(true, setting);
}
...
TEST_F(BraveShieldsUtilTest, SetAdControlType_Default) {
  ...
  /* ALLOW */
  wootz_adblock::SetAdControlType(map, ControlType::ALLOW, GURL());
  setting =
      map->GetContentSetting(GURL(), GURL(), ContentSettingsType::BRAVE_ADS);
  EXPECT_EQ(CONTENT_SETTING_ALLOW, setting);
  ...
  /* BLOCK */
  wootz_adblock::SetAdControlType(map, ControlType::BLOCK, GURL());
  setting =
      map->GetContentSetting(GURL(), GURL(), ContentSettingsType::BRAVE_ADS);
  EXPECT_EQ(CONTENT_SETTING_BLOCK, setting);
...
TEST_F(BraveShieldsUtilTest, SetAdControlType_ForOrigin) {
  ...
  wootz_adblock::SetAdControlType(map, ControlType::ALLOW,
                                  GURL("http://brave.com"));
  // setting should apply to origin
  auto setting = map->GetContentSetting(GURL("http://brave.com"), GURL(),
                                        ContentSettingsType::BRAVE_ADS);
...
TEST_F(BraveShieldsUtilTest, GetAdControlType_Default) {
  ...
  auto setting = wootz_adblock::GetAdControlType(map, GURL());
  ...
  setting = wootz_adblock::GetAdControlType(map, GURL());
...
TEST_F(BraveShieldsUtilTest, GetAdControlType_ForOrigin) {
  ...
  auto setting = wootz_adblock::GetAdControlType(map, GURL());
  EXPECT_EQ(ControlType::BLOCK, setting);
  setting = wootz_adblock::GetAdControlType(map, GURL("http://brave.com"));
  EXPECT_EQ(ControlType::BLOCK, setting);
  setting = wootz_adblock::GetAdControlType(map, GURL("https://brave.com"));
  EXPECT_EQ(ControlType::BLOCK, setting);
...
TEST_F(BraveShieldsUtilTest, SetCookieControlType_Default) {
  ...
  /* ALLOW */
  wootz_adblock::SetCookieControlType(map, profile()->GetPrefs(),
...
TEST_F(BraveShieldsUtilTest, SetCookieControlType_ForOrigin) {
  ...
  wootz_adblock::SetCookieControlType(
      map, profile()->GetPrefs(), ControlType::ALLOW, GURL("http://brave.com"));
  // override should apply to origin
  auto setting = wootz_adblock::GetCookieControlType(map, cookies.get(),
                                                     GURL("http://brave.com"));
  EXPECT_EQ(ControlType::ALLOW, setting);

  // override should also apply to different scheme
  setting = wootz_adblock::GetCookieControlType(map, cookies.get(),
                                                GURL("http://brave.com"));
  EXPECT_EQ(ControlType::ALLOW, setting);
}
...
TEST_F(BraveShieldsUtilTest, GetCookieControlType_Default) {
  ...
  auto setting =
      wootz_adblock::GetCookieControlType(map, cookies.get(), GURL());
  ...
  wootz_adblock::SetCookieControlType(map, profile()->GetPrefs(),
                                      ControlType::ALLOW, GURL());
  setting = wootz_adblock::GetCookieControlType(map, cookies.get(), GURL());
  EXPECT_EQ(ControlType::ALLOW, setting);
  setting = wootz_adblock::GetCookieControlType(map, cookies.get(),
                                                GURL("http://brave.com"));
  EXPECT_EQ(ControlType::ALLOW, setting);
...
TEST_F(BraveShieldsUtilTest, SetFingerprintingControlType_Default) {
  ...
  scoped_feature_list.InitAndEnableFeature(
      wootz_adblock::features::kBraveShowStrictFingerprintingMode);

  auto* map = HostContentSettingsMapFactory::GetForProfile(profile());
  // setting should be default to start with
  auto type = wootz_adblock::GetFingerprintingControlType(map, GURL());
  EXPECT_EQ(ControlType::DEFAULT, type);
  type = wootz_adblock::GetFingerprintingControlType(map,
                                                     GURL("http://brave.com"));
  EXPECT_EQ(ControlType::DEFAULT, type);

  /* ALLOW */
  wootz_adblock::SetFingerprintingControlType(map, ControlType::ALLOW, GURL());
  type = wootz_adblock::GetFingerprintingControlType(map, GURL());
  ...
  /* BLOCK */
  wootz_adblock::SetFingerprintingControlType(map, ControlType::BLOCK, GURL());
  type = wootz_adblock::GetFingerprintingControlType(map, GURL());
  ...
  /* DEFAULT */
  wootz_adblock::SetFingerprintingControlType(map, ControlType::DEFAULT,
                                              GURL());
  type = wootz_adblock::GetFingerprintingControlType(map, GURL());
  EXPECT_EQ(ControlType::DEFAULT, type);
...
TEST_F(BraveShieldsUtilTest, SetFingerprintingControlType_ForOrigin) {
  ...
  wootz_adblock::SetFingerprintingControlType(map, ControlType::ALLOW,
                                              GURL("http://brave.com"));
  auto type = wootz_adblock::GetFingerprintingControlType(
  ...
  wootz_adblock::SetFingerprintingControlType(map, ControlType::BLOCK,
                                              GURL("http://brave.com"));
  type = wootz_adblock::GetFingerprintingControlType(map,
                                                     GURL("http://brave.com"));
  EXPECT_EQ(ControlType::BLOCK, type);
...
TEST_F(BraveShieldsUtilTest, SetNoScriptControlType_Default) {
  ...
  /* BLOCK */
  wootz_adblock::SetNoScriptControlType(map, ControlType::BLOCK, GURL());
  setting =
      map->GetContentSetting(GURL(), GURL(), ContentSettingsType::JAVASCRIPT);
  EXPECT_EQ(CONTENT_SETTING_BLOCK, setting);
  ...
  /* ALLOW */
  wootz_adblock::SetNoScriptControlType(map, ControlType::ALLOW, GURL());
  setting =
      map->GetContentSetting(GURL(), GURL(), ContentSettingsType::JAVASCRIPT);
  EXPECT_EQ(CONTENT_SETTING_ALLOW, setting);
...
TEST_F(BraveShieldsUtilTest, SetNoScriptControlType_ForOrigin) {
  ...
  wootz_adblock::SetNoScriptControlType(map, ControlType::BLOCK,
                                        GURL("http://brave.com"));
  // setting should apply to origin
  auto setting = map->GetContentSetting(GURL("http://brave.com"), GURL(),
                                        ContentSettingsType::JAVASCRIPT);
...
TEST_F(BraveShieldsUtilTest, GetNoScriptControlType_Default) {
  ...
  auto setting = wootz_adblock::GetNoScriptControlType(map, GURL());
  EXPECT_EQ(ControlType::ALLOW, setting);
  ...
  setting = wootz_adblock::GetNoScriptControlType(map, GURL());
  EXPECT_EQ(ControlType::BLOCK, setting);
...
TEST_F(BraveShieldsUtilTest, GetNoScriptControlType_ForOrigin) {
  ...
  EXPECT_EQ(ControlType::ALLOW, setting);
  setting =
      wootz_adblock::GetNoScriptControlType(map, GURL("http://brave.com"));
  EXPECT_EQ(ControlType::ALLOW, setting);
  setting =
      wootz_adblock::GetNoScriptControlType(map, GURL("https://brave.com"));
  EXPECT_EQ(ControlType::ALLOW, setting);
...
TEST_F(BraveShieldsUtilTest, GetDomainBlockingType_ShieldsDown) {
  ...
  wootz_adblock::SetBraveShieldsEnabled(map, false, url);
...
TEST_F(BraveShieldsUtilTest, GetDomainBlockingType_ControlTypes) {
  ...
  for (const auto& test_case : kTestCases) {
    ...
    wootz_adblock::SetAdControlType(map, test_case.ad_control_type, url);
