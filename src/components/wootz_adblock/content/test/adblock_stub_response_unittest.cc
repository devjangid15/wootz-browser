...
#include "components/wootz_adblock/content/browser/adblock_stub_response.h"
...
TEST(AdBlockStubResponse, ScriptDataURL) {
  ...
  wootz_adblock::MakeStubResponse(data_url, request, &resource_response, &data);
...
TEST(AdBlockStubResponse, AcceptHeaderUsedNoDataURL) {
  ...
  auto resource_response = network::mojom::URLResponseHead::New();
  wootz_adblock::MakeStubResponse("", request, &resource_response, &data);
  ASSERT_EQ(data, "");
  ASSERT_EQ(resource_response->mime_type, "text/xml");
}
