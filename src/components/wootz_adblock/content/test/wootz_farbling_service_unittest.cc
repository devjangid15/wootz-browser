...
#include "components/wootz_adblock/content/browser/brave_farbling_service.h"
...
TEST_F(BraveFarblingServiceTest, ShieldsDown) {
  ...
  wootz_adblock::SetBraveShieldsEnabled(settings_map_.get(), false, url);
  brave::FarblingPRNG prng;
...
TEST_F(BraveFarblingServiceTest, FingerprintingAllowed) {
  ...
  wootz_adblock::SetFingerprintingControlType(
      settings_map_.get(), wootz_adblock::ControlType::ALLOW, url);
  brave::FarblingPRNG prng;
  EXPECT_FALSE(farbling_service()->MakePseudoRandomGeneratorForURL(url, &prng));
}
