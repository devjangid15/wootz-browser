...
static void JNI_ConnectAccountFragment_ConnectAccount(
  ...
  wootz_wallet::mojom::CoinType coin =
      static_cast<wootz_wallet::mojom::CoinType>(account_id_coin);
  CHECK(wootz_wallet::mojom::IsKnownEnumValue(coin));

  auto request_type = wootz_wallet::CoinTypeToPermissionRequestType(coin);
  auto permission = wootz_wallet::CoinTypeToPermissionType(coin);
