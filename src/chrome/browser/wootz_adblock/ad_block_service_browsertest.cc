...
#include "chrome/browser/wootz_adblock/ad_block_service_browsertest.h"
...
#include "chrome/browser/brave_browser_process.h"
#include "chrome/browser/net/brave_ad_block_tp_network_delegate_helper.h"
#include "components/wootz_adblock/content/browser/ad_block_custom_filters_provider.h"
...
#include "components/wootz_adblock/content/browser/ad_block_subscription_service_manager.h"
#include "components/wootz_adblock/content/browser/ad_block_subscription_service_manager_observer.h"
#include "components/wootz_adblock/content/browser/wootz_adblock_util.h"
#include "components/wootz_adblock/content/test/engine_test_observer.h"
...
#include "components/wootz_adblock/core/browser/ad_block_default_resource_provider.h"
#include "components/wootz_adblock/core/browser/filter_list_catalog_entry.h"
#include "components/wootz_adblock/core/common/brave_shield_constants.h"
#include "components/wootz_adblock/core/common/features.h"
#include "components/wootz_adblock/core/common/pref_names.h"
#include "components/constants/brave_paths.h"
#include "components/constants/pref_names.h"
#include "components/de_amp/common/pref_names.h"
#include "components/playlist/common/buildflags/buildflags.h"
...
#include "chrome/browser/playlist/playlist_service_factory.h"
#include "components/playlist/browser/playlist_background_web_contentses.h"
#include "components/playlist/browser/playlist_service.h"
#include "components/playlist/common/features.h"
#endif

#if BUILDFLAG(ENABLE_SPEEDREADER) && !BUILDFLAG(IS_ANDROID)
#include "chrome/browser/speedreader/speedreader_service_factory.h"
#include "components/speedreader/speedreader_service.h"
...
using wootz_adblock::features::kBraveAdblockCnameUncloaking;
using wootz_adblock::features::kBraveAdblockCollapseBlockedElements;
using wootz_adblock::features::kBraveAdblockCosmeticFiltering;
using wootz_adblock::features::kBraveAdblockDefault1pBlocking;
using wootz_adblock::features::kBraveAdblockProceduralFiltering;
using wootz_adblock::features::kBraveAdblockScriptletDebugLogs;
using wootz_adblock::features::kCosmeticFilteringJsPerformance;
...
void AdBlockServiceTest::SetUpOnMainThread() {
  ...
  // behavior.
  wootz_adblock::SetCosmeticFilteringControlType(
      content_settings(), wootz_adblock::ControlType::BLOCK, GURL());
}
...
void AdBlockServiceTest::AddNewRules(const std::string& rules,
  ...
  auto source_provider = std::make_unique<wootz_adblock::TestFiltersProvider>(
      rules, first_party_protections, permission_mask);
  source_providers_.push_back(std::move(source_provider));

  wootz_adblock::AdBlockService* ad_block_service =
...
void AdBlockServiceTest::UpdateAdBlockResources(const std::string& resources) {
  ...
  wootz_adblock::AdBlockService* service =
      g_brave_browser_process->ad_block_service();
...
void AdBlockServiceTest::EnableDeveloperMode(bool enabled) {
  profile()->GetPrefs()->SetBoolean(wootz_adblock::prefs::kAdBlockDeveloperMode,
                                    enabled);
}
...
void AdBlockServiceTest::InstallComponent(
    const wootz_adblock::FilterListCatalogEntry& catalog_entry) {
  auto* service = g_brave_browser_process->ad_block_service();
  std::vector<wootz_adblock::FilterListCatalogEntry> filter_list_catalog(
...
void AdBlockServiceTest::InstallDefaultAdBlockComponent() {
  auto catalog_entry = wootz_adblock::FilterListCatalogEntry(
...
void AdBlockServiceTest::ShieldsDown(const GURL& url) {
  wootz_adblock::SetBraveShieldsEnabled(content_settings(), false, url);
}

void AdBlockServiceTest::DisableAggressiveMode() {
  wootz_adblock::SetCosmeticFilteringControlType(
      content_settings(), wootz_adblock::ControlType::BLOCK_THIRD_PARTY,
      GURL());
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, DefaultStartupWithCookieList) {
  ...
  InstallRegionalAdBlockComponent(wootz_adblock::kCookieListUuid, true);
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SubFrameShieldsOff) {
  ...
  content::RunAllTasksUntilIdle();
  EXPECT_EQ(profile()->GetPrefs()->GetUint64(kAdsBlocked), 0ULL);
  wootz_adblock::ResetBraveShieldsEnabled(content_settings(), url);
}
...
class TestAdBlockSubscriptionServiceManagerObserver
    : public wootz_adblock::AdBlockSubscriptionServiceManagerObserver {
 public:
  ...
  explicit TestAdBlockSubscriptionServiceManagerObserver(
      wootz_adblock::AdBlockSubscriptionServiceManager* sub_service_manager)
 ...
 private:
  ...
  raw_ptr<wootz_adblock::AdBlockSubscriptionServiceManager>
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SocialButttonAdBlockTagTest) {
  UpdateAdBlockInstanceWithRules(
      base::StrCat({"||example.com^$tag=", wootz_adblock::kFacebookEmbeds}));
  GURL tab_url = embedded_test_server()->GetURL("b.com", kAdBlockTestPage);
  g_brave_browser_process->ad_block_service()->EnableTag(
      wootz_adblock::kFacebookEmbeds, true);
  WaitForAdBlockServiceThreads();
  GURL resource_url =
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ResetPreservesTags) {
  ...
  WaitForAdBlockServiceThreads();
  UpdateAdBlockInstanceWithRules("");
  AssertTagExists(wootz_adblock::kFacebookEmbeds, true);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, TagPrefsControlTags) {
  ...
  AssertTagExists(wootz_adblock::kFacebookEmbeds, true);
  AssertTagExists(wootz_adblock::kTwitterEmbeds, true);
  AssertTagExists(wootz_adblock::kLinkedInEmbeds, false);

  // Toggling prefs once is reflected in the adblock client.
  prefs->SetBoolean(wootz_adblock::prefs::kLinkedInEmbedControlType, true);
  WaitForAdBlockServiceThreads();
  AssertTagExists(wootz_adblock::kFacebookEmbeds, true);
  AssertTagExists(wootz_adblock::kTwitterEmbeds, true);
  AssertTagExists(wootz_adblock::kLinkedInEmbeds, true);

  prefs->SetBoolean(wootz_adblock::prefs::kFBEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(wootz_adblock::kFacebookEmbeds, false);
  AssertTagExists(wootz_adblock::kTwitterEmbeds, true);
  AssertTagExists(wootz_adblock::kLinkedInEmbeds, true);

  prefs->SetBoolean(wootz_adblock::prefs::kTwitterEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(wootz_adblock::kFacebookEmbeds, false);
  AssertTagExists(wootz_adblock::kTwitterEmbeds, false);
  AssertTagExists(wootz_adblock::kLinkedInEmbeds, true);

  // Toggling prefs back is reflected in the adblock client.
  prefs->SetBoolean(wootz_adblock::prefs::kLinkedInEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(wootz_adblock::kFacebookEmbeds, false);
  AssertTagExists(wootz_adblock::kTwitterEmbeds, false);
  AssertTagExists(wootz_adblock::kLinkedInEmbeds, false);

  prefs->SetBoolean(wootz_adblock::prefs::kFBEmbedControlType, true);
  WaitForAdBlockServiceThreads();
  AssertTagExists(wootz_adblock::kFacebookEmbeds, true);
  AssertTagExists(wootz_adblock::kTwitterEmbeds, false);
  AssertTagExists(wootz_adblock::kLinkedInEmbeds, false);

  prefs->SetBoolean(wootz_adblock::prefs::kTwitterEmbedControlType, true);
  WaitForAdBlockServiceThreads();
  AssertTagExists(wootz_adblock::kFacebookEmbeds, true);
  AssertTagExists(wootz_adblock::kTwitterEmbeds, true);
  AssertTagExists(wootz_adblock::kLinkedInEmbeds, false);
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, HiddenListsNotPresented) {
  std::vector<wootz_adblock::FilterListCatalogEntry> filter_list_catalog;
  filter_list_catalog.push_back(wootz_adblock::FilterListCatalogEntry(
...
IN_PROC_BROWSER_TEST_F(CosmeticFilteringPlaylistFlagEnabledTest,
  ...
  wootz_adblock::SetCosmeticFilteringControlType(
      content_settings(), wootz_adblock::ControlType::ALLOW, url);
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringDisabled) {
  wootz_adblock::SetCosmeticFilteringControlType(
      content_settings(), wootz_adblock::ControlType::ALLOW, GURL());
  UpdateAdBlockInstanceWithRules(
      "b.com###ad-banner\n"
      "##.ad");
 ...
 public:
  explicit CookieListPrefObserver(PrefService* local_state) {
    ...
    pref_change_registrar_.Add(
        wootz_adblock::prefs::kAdBlockRegionalFilters,
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ListEnabled) {
  InstallRegionalAdBlockComponent(wootz_adblock::kCookieListUuid, false);
  ...
  // Disable the filter list.
  {
    ...
    g_brave_browser_process->ad_block_service()
        ->component_service_manager()
        ->EnableFilterList(wootz_adblock::kCookieListUuid, false);
    pref_observer.Wait();
  }
