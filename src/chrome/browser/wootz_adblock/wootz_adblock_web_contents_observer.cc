...
#include "components/brave_perf_predictor/browser/perf_predictor_tab_helper.h"
...
#include "components/wootz_adblock/core/common/shields_settings.mojom.h"
...
void BraveShieldsWebContentsObserver::DispatchBlockedEventForWebContents(
  ...
  auto* shields_data_ctrlr =
      wootz_adblock::BraveShieldsTabHelper::FromWebContents(web_contents);
...
void BraveShieldsWebContentsObserver::OnJavaScriptAllowedOnce(
  ...
  DispatchAllowedOnceEventForWebContents(
      wootz_adblock::kJavaScript, base::UTF16ToUTF8(details), web_contents);
#endif
}
...
void BraveShieldsWebContentsObserver::OnJavaScriptBlocked(
  ...
  DispatchBlockedEventForWebContents(wootz_adblock::kJavaScript,
...
void BraveShieldsWebContentsObserver::SendShieldsSettings(
  ...
  const wootz_adblock::mojom::FarblingLevel farbling_level =
      wootz_adblock::GetFarblingLevel(host_content_settings_map, primary_url);
  const base::Token farbling_token =
      farbling_level != wootz_adblock::mojom::FarblingLevel::OFF
          ? wootz_adblock::GetFarblingToken(host_content_settings_map,
                                            primary_url)
          : base::Token();
  ...
  mojo::AssociatedRemote<wootz_adblock::mojom::BraveShields> agent;
  rfh->GetRemoteAssociatedInterfaces()->GetInterface(&agent);
  agent->SetShieldsSettings(wootz_adblock::mojom::ShieldsSettings::New(
      farbling_level, farbling_token, allowed_scripts_,
      wootz_adblock::IsReduceLanguageEnabledForProfile(pref_service)));
}
