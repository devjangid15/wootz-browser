...
class BraveShieldsWebContentsObserverBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    // wootz_adblock::mojom::BraveShieldsHost interface for testing purposes.
    ...
    wootz_adblock_web_contents_observer_ =
  ...
  std::vector<GURL> GetBlockedJsList() {
    return wootz_adblock::BraveShieldsTabHelper::FromWebContents(
               GetWebContents())
        ->GetBlockedJsList();
  }
  ...
  TestBraveShieldsWebContentsObserver* wootz_adblock_web_contents_observer() {
    return wootz_adblock_web_contents_observer_;
  }

 private:
  raw_ptr<HostContentSettingsMap> content_settings_ = nullptr;
  raw_ptr<TestBraveShieldsWebContentsObserver>
      wootz_adblock_web_contents_observer_ = nullptr;
};
...
IN_PROC_BROWSER_TEST_F(BraveShieldsWebContentsObserverBrowserTest,
  ...
  EXPECT_EQ(wootz_adblock_web_contents_observer()->block_javascript_count(), 0);
  ...
  wootz_adblock_web_contents_observer()->Reset();
  GetWebContents()->GetController().Reload(content::ReloadType::NORMAL, true);
  EXPECT_TRUE(WaitForLoadStop(GetWebContents()));
  EXPECT_GT(wootz_adblock_web_contents_observer()->block_javascript_count(), 0);
...
IN_PROC_BROWSER_TEST_F(BraveShieldsWebContentsObserverBrowserTest,
  ...
  // Allow One Script
  wootz_adblock_web_contents_observer()->AllowScriptsOnce(
  ...
  wootz_adblock_web_contents_observer()->BlockAllowedScripts(
...
IN_PROC_BROWSER_TEST_F(BraveShieldsWebContentsObserverBrowserTest,
  ...
  EXPECT_TRUE(WaitForLoadStop(GetWebContents()));
  EXPECT_EQ(wootz_adblock_web_contents_observer()->block_javascript_count(), 4);
  wootz_adblock_web_contents_observer()->Reset();
  ...
  EXPECT_EQ(GetBlockedJsList().size(), 1u);
  EXPECT_EQ(GetAllowedJsList().size(), 1u);
  EXPECT_EQ(wootz_adblock_web_contents_observer()->block_javascript_count(), 3);
  wootz_adblock_web_contents_observer()->Reset();
