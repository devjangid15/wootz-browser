...
class TestTxServiceObserver : public wootz_wallet::mojom::TxServiceObserver {
 public:
...
class SendOrSignTransactionBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    auto* wallet_service =
        WootzWalletServiceFactory::GetServiceForContext(browser()->profile());
  ...
  void AddEthereumPermission(const mojom::AccountIdPtr& account_id) {
    EXPECT_TRUE(permissions::WootzWalletPermissionContext::AddPermission(
        blink::PermissionType::WOOTZ_ETHEREUM, browser()->profile(),
  ...
  void SetNetworkForTesting(const std::string& chain_id,
    ...
    if (chain && !skip_rpc_url_override) {
      ...
      browser()->profile()->GetPrefs()->ClearPref(kWootzWalletCustomNetworks);
      chain->rpc_endpoints =
          std::vector<GURL>({https_server_for_rpc()->base_url()});
...
IN_PROC_BROWSER_TEST_F(SendOrSignTransactionBrowserTest, CallViaProxy) {
  ...
  EXPECT_TRUE(
      EvalJs(web_contents(), "getIsWootzWalletViaProxy()").ExtractBool());
}
