...
class TestJsonRpcServiceObserver
 ...
 public:
  TestJsonRpcServiceObserver(base::OnceClosure callback,
                             ...
                             wootz_wallet::mojom::CoinType expected_coin,
  ...
  void ChainChangedEvent(const std::string& chain_id,
                         wootz_wallet::mojom::CoinType coin,
  ...
  wootz_wallet::mojom::CoinType expected_coin_;
...
class WootzWalletEthereumChainTest : public InProcessBrowserTest {
 public:
  WootzWalletEthereumChainTest() {
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_->RegisterRequestHandler(
        base::BindRepeating(&WootzWalletEthereumChainTest::HandleChainRequest,
  ...
  GURL GetWalletEthereumChainPageURL(const std::string& host = "a.com",
    ...
    auto url =
        https_server()->GetURL(host, "/wootz_wallet_ethereum_chain.html");
    return url.ReplaceComponents(replacements);
  }

  wootz_wallet::JsonRpcService* GetJsonRpcService() {
    return wootz_wallet::WootzWalletServiceFactory::GetInstance()
        ->GetServiceForContext(browser()->profile())
        ->json_rpc_service();
  }

  std::vector<wootz_wallet::mojom::NetworkInfoPtr> GetAllEthCustomChains() {
...
IN_PROC_BROWSER_TEST_F(WootzWalletEthereumChainTest, AddEthereumChainApproved) {
  ...
  auto* tab_helper =
      wootz_wallet::WootzWalletTabHelper::FromWebContents(contents);
  tab_helper->SetShowBubbleCallbackForTesting(loop.QuitClosure());
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  WaitForLoadStop(contents);
  loop.Run();

  ASSERT_TRUE(wootz_wallet::WootzWalletTabHelper::FromWebContents(contents)
                  ->IsShowingBubble());
  CallAndWaitForEthereumChainRequestCompleted(
      kSomeChainId, true, wootz_wallet::mojom::CoinType::ETH, "");
  ...
  EXPECT_EQ(GetJsonRpcService()->GetChainIdSync(
                wootz_wallet::mojom::CoinType::ETH, origin),
            kSomeChainId);
}

IN_PROC_BROWSER_TEST_F(WootzWalletEthereumChainTest, AddEthereumChainRejected) {
  GURL url = GetWalletEthereumChainPageURL();
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  base::RunLoop loop;
...
IN_PROC_BROWSER_TEST_F(WootzWalletEthereumChainTest, AddChainSameOrigin) {
  GURL url = GetWalletEthereumChainPageURL();
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  base::RunLoop loop;
...
IN_PROC_BROWSER_TEST_F(WootzWalletEthereumChainTest,
  ...
  base::RunLoop loop;
  auto* tab_helperA =
      wootz_wallet::WootzWalletTabHelper::FromWebContents(contentsA);
  ...
  auto* tab_helperB =
      wootz_wallet::WootzWalletTabHelper::FromWebContents(web_contentsB);
...
IN_PROC_BROWSER_TEST_F(WootzWalletEthereumChainTest,
  ...
  CallAndWaitForEthereumChainRequestCompleted(
      "0x11", true, wootz_wallet::mojom::CoinType::ETH, "");
...
IN_PROC_BROWSER_TEST_F(WootzWalletEthereumChainTest, AddDifferentChainsSwitch) {
  ASSERT_TRUE(GetAllEthCustomChains().empty());
...
IN_PROC_BROWSER_TEST_F(WootzWalletEthereumChainTest, AddChainAndCloseTab) {
  GURL urlA = GetWalletEthereumChainPageURL();
  content::WebContents* contentsA =
      browser()->tab_strip_model()->GetActiveWebContents();
  auto* tab_helperA =
...
IN_PROC_BROWSER_TEST_F(WootzWalletEthereumChainTest, AddBrokenChain) {
  GURL url = GetWalletEthereumChainPageURL();
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  base::RunLoop loop;
...
IN_PROC_BROWSER_TEST_F(WootzWalletEthereumChainTest, CheckIncognitoTab) {
  GURL url = GetWalletEthereumChainPageURL();
