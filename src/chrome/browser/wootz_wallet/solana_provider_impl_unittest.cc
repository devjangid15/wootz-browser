 ...
 public:
  ...
  void SetUp() override {
    ...
    wootz_wallet_service_ = std::make_unique<WootzWalletService>(
        shared_url_loader_factory_,
        WootzWalletServiceDelegate::Create(browser_context()),
  ...
  void AddSolanaPermission(const mojom::AccountIdPtr& account_id) {
    EXPECT_TRUE(permissions::WootzWalletPermissionContext::AddPermission(
        blink::PermissionType::WOOTZ_SOLANA, browser_context(), GetOrigin(),
        account_id->address));
  }
  ...
  std::string SignMessage(
    ...
    if (run_notify) {
      wootz_wallet_service_->NotifySignMessageRequestProcessed(
          approve, wootz_wallet_service_->sign_message_id_ - 1,
  ...
  std::vector<uint8_t> SignTransaction(
    ...
    if (run_notify) {
      wootz_wallet_service_->SetSignTransactionRequestAddedCallbackForTesting(
          base::BindLambdaForTesting([&]() {
            wootz_wallet_service_->NotifySignTransactionRequestProcessed(
                approve, wootz_wallet_service_->sign_transaction_id_ - 1,
                std::move(hw_sig), err_in);
          }));
    }
              ...
              base::BindLambdaForTesting([&]() {
                wootz_wallet_service_
                    ->NotifySignAllTransactionsRequestProcessed(
                        approve,
                        wootz_wallet_service_->sign_all_transactions_id_ - 1,
...
TEST_F(SolanaProviderImplUnitTest, Connect) {
  ...
  // permissions::WootzWalletPermissionContext::RequestPermissions failed
  ...
  map->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::WOOTZ_SOLANA, CONTENT_SETTING_BLOCK);
  account = Connect(std::nullopt, &error, &error_message);
  EXPECT_TRUE(account.empty());
  EXPECT_EQ(error, mojom::SolanaProviderError::kUserRejectedRequest);
  EXPECT_EQ(error_message,
  ...
  // again.
  map->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::WOOTZ_SOLANA, CONTENT_SETTING_DEFAULT);
  account = Connect(std::nullopt, &error, &error_message);
  EXPECT_EQ(account, added_account->address);
  EXPECT_EQ(error, mojom::SolanaProviderError::kSuccess);
  EXPECT_TRUE(error_message.empty());
...
TEST_F(SolanaProviderImplUnitTest, SignMessage) {
  ...
  EXPECT_EQ(requests[0]->sign_data->get_solana_sign_data()->message, "WOOTZ");
  EXPECT_EQ(requests[1]->sign_data->get_solana_sign_data()->message, "WOOTZ");
  EXPECT_EQ(requests[2]->sign_data->get_solana_sign_data()->message,
            "0x4252415645");
  EXPECT_EQ(requests[3]->sign_data->get_solana_sign_data()->message, "WOOTZ");
}
