...
#include "components/wootz_wallet/browser/bitcoin/bitcoin_test_utils.h"
...
constexpr char kWootzUrl[] = "https://wootz.com";
...
class TestWootzWalletServiceObserver
 ...
 private:
  mojom::DefaultWallet default_ethereum_wallet_ =
      mojom::DefaultWallet::WootzWalletPreferExtension;
...
class WootzWalletServiceUnitTest : public testing::Test {
 public:
  WootzWalletServiceUnitTest()
      : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME),
        shared_url_loader_factory_(
            base::MakeRefCounted<network::WeakWrapperSharedURLLoaderFactory>(
                &url_loader_factory_)) {}

  ~WootzWalletServiceUnitTest() override = default;

 protected:
  void SetUp() override {
    scoped_feature_list_.InitAndEnableFeature(
        features::kWootzWalletBitcoinFeature);
    ...
    builder.AddTestingFactory(
        ...
        base::BindRepeating(
            [](scoped_refptr<network::SharedURLLoaderFactory>
              ...
              return std::make_unique<WootzWalletService>(
                  shared_url_loader_factory,
                  WootzWalletServiceDelegate::Create(profile),
                  profile->GetPrefs(), local_state);
            },
            shared_url_loader_factory_, local_state_->Get()));
    profile_ = builder.Build();
    service_ = wootz_wallet::WootzWalletServiceFactory::GetServiceForContext(
        profile_.get());
    ...
    observer_ = std::make_unique<TestWootzWalletServiceObserver>();
    service_->AddObserver(observer_->GetReceiver());

    profile_->SetPermissionControllerDelegate(
        base::WrapUnique(static_cast<permissions::WootzPermissionManager*>(
  ...
  void AddSuggestToken(mojom::BlockchainTokenPtr suggested_token,
    ...
    mojom::AddSuggestTokenRequestPtr request =
        mojom::AddSuggestTokenRequest::New(
            MakeOriginInfo(url::Origin::Create(GURL(kWootzUrl))),
  ...
  raw_ptr<WootzWalletService> service_ = nullptr;
  ...
  std::unique_ptr<TestWootzWalletServiceObserver> observer_;
...
TEST_F(WootzWalletServiceUnitTest, GetUserAssets) {
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(WootzWalletServiceUnitTest, GetUserAssetsAlwaysHasNativeTokensForBtc) {
  GetPrefs()->SetList(kWootzWalletUserAssetsList, base::Value::List());
...
TEST_F(WootzWalletServiceUnitTest, GetUserAssetsAlwaysHasNativeTokensForZec) {
  GetPrefs()->SetList(kWootzWalletUserAssetsList, base::Value::List());
...
TEST_F(WootzWalletServiceUnitTest, DefaultAssets) {
...
TEST_F(WootzWalletServiceUnitTest, AddUserAsset) {
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(WootzWalletServiceUnitTest, AddUserAssetNfts) {
...
TEST_F(WootzWalletServiceUnitTest, RemoveUserAsset) {
  mojom::BlockchainTokenPtr token1 = GetToken1();
  mojom::BlockchainTokenPtr token2 = GetToken2();
  mojom::BlockchainTokenPtr token1_0x1 = GetToken1();
  token1_0x1->chain_id = "0x1";
...
TEST_F(WootzWalletServiceUnitTest, SetUserAssetVisible) {
  mojom::BlockchainTokenPtr token1 = GetToken1();
  mojom::BlockchainTokenPtr token2 = GetToken2();
  mojom::BlockchainTokenPtr token1_0x1 = GetToken1();
  token1_0x1->chain_id = "0x1";
...
TEST_F(WootzWalletServiceUnitTest, SetAssetSpamStatus) {
  mojom::BlockchainTokenPtr token1 = GetToken1();
...
TEST_F(WootzWalletServiceUnitTest, GetAndSetDefaultEthereumWallet) {
  SetDefaultEthereumWallet(mojom::DefaultWallet::WootzWallet);
  EXPECT_EQ(GetDefaultEthereumWallet(), mojom::DefaultWallet::WootzWallet);
  ...
  SetDefaultEthereumWallet(mojom::DefaultWallet::WootzWalletPreferExtension);
...
TEST_F(WootzWalletServiceUnitTest, GetAndSetDefaultSolanaWallet) {
  SetDefaultSolanaWallet(mojom::DefaultWallet::WootzWallet);
  EXPECT_EQ(GetDefaultSolanaWallet(), mojom::DefaultWallet::WootzWallet);

  SetDefaultSolanaWallet(mojom::DefaultWallet::None);
  EXPECT_EQ(GetDefaultSolanaWallet(), mojom::DefaultWallet::None);

  SetDefaultSolanaWallet(mojom::DefaultWallet::WootzWalletPreferExtension);
...
TEST_F(WootzWalletServiceUnitTest, GetAndSetDefaultBaseCurrency) {
...
TEST_F(WootzWalletServiceUnitTest, GetAndSetDefaultBaseCryptocurrency) {
...
TEST_F(WootzWalletServiceUnitTest, EthAddRemoveSetUserAssetVisible) {
  mojom::BlockchainTokenPtr eth_0xaa36a7_token = GetEthToken();
  eth_0xaa36a7_token->chain_id = "0xaa36a7";
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(WootzWalletServiceUnitTest, NetworkListChangedEvent) {
  ...
  observer_->Reset();
  {
    ScopedDictPrefUpdate update(GetPrefs(), kWootzWalletCustomNetworks);
...
TEST_F(WootzWalletServiceUnitTest,
...
TEST_F(WootzWalletServiceUnitTest, AddCustomNetwork) {
  json_rpc_service_->SetSkipEthChainIdValidationForTesting(true);
  ...
  // kWootzWalletCustomNetworks should be updated with new chains.
  ASSERT_EQ(2u,
  ...
  // kWootzWalletUserAssets.
  const auto& asset_list = GetPrefs()->GetList(kWootzWalletUserAssetsList);
...
TEST_F(WootzWalletServiceUnitTest, AddCustomNetworkTwice) {
  json_rpc_service_->SetSkipEthChainIdValidationForTesting(true);
...
TEST_F(WootzWalletServiceUnitTest, ERC721TokenAddRemoveSetUserAssetVisible) {
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(WootzWalletServiceUnitTest, SolanaTokenUserAssetsAPI) {
  std::vector<mojom::BlockchainTokenPtr> tokens;
...
TEST_F(WootzWalletServiceUnitTest, MigrateEip1559ForCustomNetworks) {
  ...
  ASSERT_TRUE(
      GetPrefs()->GetBoolean(kWootzWalletEip1559ForCustomNetworksMigrated));
  GetPrefs()->ClearPref(kWootzWalletEip1559ForCustomNetworksMigrated);
  ...
  char legacy_custom_networks_pref[] =
      R"( {
            ...
            "rpcUrls": [ "https://mainnet-aurora.wootz.com/" ]
          }, {
            "activeRpcEndpointIndex": 0,
            ...
            "rpcUrls": [ "https://mainnet-infura.wootz.com/" ]
          }, {
            "activeRpcEndpointIndex": 0,
  ...
  GetPrefs()->SetDict(kWootzWalletCustomNetworks,
                      base::test::ParseJsonDict(legacy_custom_networks_pref));

  WootzWalletService::MigrateEip1559ForCustomNetworks(GetPrefs());
  for (auto&& [coin_key, value] :
       GetPrefs()->GetDict(kWootzWalletCustomNetworks)) {
  ...
  EXPECT_EQ(GetPrefs()->GetDict(kWootzWalletEip1559CustomChains),
...
TEST_F(WootzWalletServiceUnitTest, MigrateDefaultHiddenNetworks) {
  // Note: The testing profile has already performed the prefs migration by the
  // time this test runs, so undo its effects here for testing purposes
  ASSERT_EQ(GetPrefs()->GetInteger(kWootzWalletDefaultHiddenNetworksVersion),
            1);
  GetPrefs()->SetInteger(kWootzWalletDefaultHiddenNetworksVersion, 0);

  WootzWalletService::MigrateHiddenNetworks(GetPrefs());
  {
    auto* list =
        GetPrefs()->GetDict(kWootzWalletHiddenNetworks).FindList("ethereum");
...
TEST_F(WootzWalletServiceUnitTest, MigrateDefaultHiddenNetworks_NoList) {
  // Note: The testing profile has already performed the prefs migration by the
  // time this test runs, so undo its effects here for testing purposes
  ASSERT_EQ(GetPrefs()->GetInteger(kWootzWalletDefaultHiddenNetworksVersion),
            1);
  ...
  {
    ScopedDictPrefUpdate update(GetPrefs(), kWootzWalletHiddenNetworks);
...
TEST_F(WootzWalletServiceUnitTest, MigrateFantomMainnetAsCustomNetwork) {
  // Note: The testing profile has already performed the prefs migration by the
  // time this test runs, so undo its effects here for testing purposes
  ASSERT_TRUE(
      GetPrefs()->GetBoolean(kWootzWalletCustomNetworksFantomMainnetMigrated));
  GetPrefs()->SetBoolean(kWootzWalletCustomNetworksFantomMainnetMigrated,
                         false);
  GetPrefs()->ClearPref(kWootzWalletCustomNetworks);
  GetPrefs()->ClearPref(kWootzWalletSelectedNetworksPerOrigin);
  ...
  GetPrefs()->Set(kWootzWalletSelectedNetworksPerOrigin, *selected_networks);

  EXPECT_FALSE(
      network_manager_->CustomChainExists("0xfa", mojom::CoinType::ETH));

  WootzWalletService::MigrateFantomMainnetAsCustomNetwork(GetPrefs());
  ...
  GetPrefs()->Set(kWootzWalletSelectedNetworks, *default_networks);
  ...
  EXPECT_EQ(
      *GetPrefs()->GetDict(kWootzWalletSelectedNetworks).FindString("ethereum"),
      "0xfa");
  ...
  ASSERT_TRUE(GetPrefs()->HasPrefPath(kWootzWalletCustomNetworks));
  auto* custom_networks =
      GetPrefs()->GetDict(kWootzWalletCustomNetworks).FindList("ethereum");
...
TEST_F(WootzWalletServiceUnitTest, MigrateGoerliNetwork) {
  // Note: The testing profile has already performed the prefs migration by the
  // time this test runs, so undo its effects here for testing purposes
  ASSERT_TRUE(GetPrefs()->GetBoolean(kWootzWalletGoerliNetworkMigrated));
  GetPrefs()->SetBoolean(kWootzWalletGoerliNetworkMigrated, false);
  GetPrefs()->ClearPref(kWootzWalletSelectedNetworksPerOrigin);
  ...
  ASSERT_FALSE(GetPrefs()->GetBoolean(kWootzWalletGoerliNetworkMigrated));
  WootzWalletService::MigrateGoerliNetwork(GetPrefs());
  EXPECT_EQ(network_manager_->GetCurrentChainId(
                mojom::CoinType::ETH,
                url::Origin::Create(GURL("https://app.uniswap.org"))),
            mojom::kSepoliaChainId);
  EXPECT_TRUE(GetPrefs()->GetBoolean(kWootzWalletGoerliNetworkMigrated));
...
TEST_F(WootzWalletServiceUnitTest, MigrateAssetsPrefToList) {
  ASSERT_FALSE(GetPrefs()->HasPrefPath(kWootzWalletUserAssetsDeprecated));
  ...
  GetPrefs()->SetDict(kWootzWalletUserAssetsDeprecated, std::move(legacy_dict));

  WootzWalletService::MigrateAssetsPrefToList(GetPrefs());

  ASSERT_FALSE(GetPrefs()->HasPrefPath(kWootzWalletUserAssetsDeprecated));
  ASSERT_TRUE(GetPrefs()->HasPrefPath(kWootzWalletUserAssetsList));
...
TEST_F(WootzWalletServiceUnitTest, OnGetImportInfo) {
  const char* new_password = "wootz1234!";
  bool success;
  std::string error_message;
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringUTF8(IDS_WOOTZ_WALLET_IMPORT_JSON_ERROR));
...
TEST_F(WootzWalletServiceUnitTest, SignMessageHardware) {
  SetupWallet();

  mojom::OriginInfoPtr origin_info =
      MakeOriginInfo(url::Origin::Create(GURL("https://wootz.com")));
...
TEST_F(WootzWalletServiceUnitTest, SignMessage) {
  SetupWallet();
...
TEST_F(WootzWalletServiceUnitTest, AddSuggestToken) {
...
TEST_F(WootzWalletServiceUnitTest, Reset) {
  SetupWallet();
  ...
  mojom::BlockchainTokenPtr token1 = GetToken1();
  EXPECT_TRUE(AddUserAsset(token1.Clone()));
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kWootzWalletUserAssetsList));
  ...
  const auto page_storage_key_callback_valid =
      [&](StoragePartition::StorageKeyMatcherFunction callback) {
        return callback.Run(blink::StorageKey::CreateFirstParty(
            url::Origin::Create(GURL(kWootzUIWalletURL))));
      };
  const auto panel_storage_key_callback_valid =
      [&](StoragePartition::StorageKeyMatcherFunction callback) {
        return callback.Run(blink::StorageKey::CreateFirstParty(
            url::Origin::Create(GURL(kWootzUIWalletPanelURL))));
      };
  ...
  auto origin = url::Origin::Create(GURL(kWootzUrl));
  auto* delegate = service_->GetDelegateForTesting();
  ASSERT_TRUE(delegate);

  ASSERT_TRUE(permissions::WootzWalletPermissionContext::AddPermission(
      blink::PermissionType::WOOTZ_ETHEREUM, profile_.get(), origin, eth_addr));
  ASSERT_TRUE(permissions::WootzWalletPermissionContext::AddPermission(
      blink::PermissionType::WOOTZ_SOLANA, profile_.get(), origin, sol_addr));
  ...
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kWootzWalletUserAssetsList));
...
TEST_F(WootzWalletServiceUnitTest, NewUserReturningMetric) {
  histogram_tester_->ExpectBucketCount(
      kWootzWalletNewUserReturningHistogramName, 0, 1);
  GetLocalState()->SetTime(kWootzWalletLastUnlockTime, base::Time::Now());

  task_environment_.FastForwardBy(base::Days(1));
  histogram_tester_->ExpectBucketCount(
      kWootzWalletNewUserReturningHistogramName, 2, 2);

  GetLocalState()->SetTime(kWootzWalletLastUnlockTime, base::Time::Now());
  task_environment_.RunUntilIdle();

  histogram_tester_->ExpectBucketCount(
      kWootzWalletNewUserReturningHistogramName, 3, 1);

  task_environment_.FastForwardBy(base::Days(6));
  histogram_tester_->ExpectBucketCount(
      kWootzWalletNewUserReturningHistogramName, 1, 1);
}

TEST_F(WootzWalletServiceUnitTest, NewUserReturningMetricMigration) {
  GetLocalState()->SetTime(kWootzWalletLastUnlockTime, base::Time::Now());

  task_environment_.RunUntilIdle();
  GetLocalState()->SetTime(kWootzWalletP3AFirstUnlockTime, base::Time());
  GetLocalState()->SetTime(kWootzWalletP3ALastUnlockTime, base::Time());
  ...
  task_environment_.FastForwardBy(base::Hours(30));
  histogram_tester_->ExpectBucketCount(
      kWootzWalletNewUserReturningHistogramName, 1, 2);
}

TEST_F(WootzWalletServiceUnitTest, LastUsageTimeMetric) {
  histogram_tester_->ExpectTotalCount(kWootzWalletLastUsageTimeHistogramName,
                                      0);

  GetLocalState()->SetTime(kWootzWalletLastUnlockTime, base::Time::Now());
  task_environment_.RunUntilIdle();

  histogram_tester_->ExpectUniqueSample(kWootzWalletLastUsageTimeHistogramName,
                                        1, 1);

  task_environment_.FastForwardBy(base::Days(7));

  histogram_tester_->ExpectBucketCount(kWootzWalletLastUsageTimeHistogramName,
                                       2, 1);
...
TEST_F(WootzWalletServiceUnitTest, GetNftDiscoveryEnabled) {
...
TEST_F(WootzWalletServiceUnitTest, SetNftDiscoveryEnabled) {
  // Default should be off
  EXPECT_FALSE(GetPrefs()->GetBoolean(kWootzWalletNftDiscoveryEnabled));
  ...
  EXPECT_TRUE(GetPrefs()->GetBoolean(kWootzWalletNftDiscoveryEnabled));
...
TEST_F(WootzWalletServiceUnitTest, SetPrivateWindowsEnabled) {
  // Default should be off
  EXPECT_FALSE(GetPrefs()->GetBoolean(kWootzWalletPrivateWindowsEnabled));

  // Setting private enabled should update the pref.
  service_->SetPrivateWindowsEnabled(true);
  base::RunLoop().RunUntilIdle();
  EXPECT_TRUE(GetPrefs()->GetBoolean(kWootzWalletPrivateWindowsEnabled));
...
TEST_F(WootzWalletServiceUnitTest, RecordGeneralUsageMetrics) {
  histogram_tester_->ExpectTotalCount(kWootzWalletMonthlyHistogramName, 0);
  histogram_tester_->ExpectTotalCount(kWootzWalletWeeklyHistogramName, 0);
  histogram_tester_->ExpectTotalCount(kWootzWalletDailyHistogramName, 0);

  GetLocalState()->SetTime(kWootzWalletLastUnlockTime, base::Time::Now());
  task_environment_.RunUntilIdle();

  histogram_tester_->ExpectUniqueSample(kWootzWalletMonthlyHistogramName, 1, 1);
  histogram_tester_->ExpectUniqueSample(kWootzWalletWeeklyHistogramName, 1, 1);
  histogram_tester_->ExpectUniqueSample(kWootzWalletDailyHistogramName, 1, 1);
  ...
  histogram_tester_->ExpectUniqueSample(kWootzWalletMonthlyHistogramName, 1, 2);
  histogram_tester_->ExpectUniqueSample(kWootzWalletWeeklyHistogramName, 1, 2);
  histogram_tester_->ExpectUniqueSample(kWootzWalletDailyHistogramName, 1, 2);
}

TEST_F(WootzWalletServiceUnitTest, GetBalanceScannerSupportedChains) {
...
TEST_F(WootzWalletServiceUnitTest, GetSimpleHashSpamNFTs) {
  ...
  GURL url = GURL(
      "https://simplehash.wallet.wootz.com/api/v0/nfts/"
...
TEST_F(WootzWalletServiceUnitTest, EnsureSelectedAccountForChain) {
  const char* new_password = "wootz1234!";
  bool success;
  std::string error_message;
  SimulateOnGetImportInfo(new_password, true,
...
TEST_F(WootzWalletServiceUnitTest, ConvertFEVMToFVMAddress) {
  {
...
TEST_F(WootzWalletServiceUnitTest, GenerateReceiveAddress_EthFilSol) {
  SetupWallet();
  ...
  for (auto& acc : accounts) {
    base::MockCallback<WootzWalletService::GenerateReceiveAddressCallback>
        callback;
...
TEST_F(WootzWalletServiceUnitTest, GenerateReceiveAddress_Btc) {
  SetupWallet();
...
TEST_F(WootzWalletServiceUnitTest, GetAnkrSupportedChainIds) {
...
TEST_F(WootzWalletServiceUnitTest, MaybeMigrateCompressedNfts) {
  GetPrefs()->SetBoolean(kWootzWalletIsCompressedNftMigrated, false);
  ...
  // Reset kWootzWalletIsCompressedNftMigrated pref, and run the migration.
  ...
  EXPECT_TRUE(tokens[2]->contract_address ==
              "BM1EG2tuxB8TS6HMwEPNztegr9qio5EyuJA1KgDWcpeW");
  EXPECT_FALSE(tokens[2]->is_compressed);
  EXPECT_TRUE(GetPrefs()->GetBoolean(kWootzWalletIsCompressedNftMigrated));
}

TEST_F(WootzWalletServiceUnitTest, GetCountryCode) {
  const struct {
