...
// IDR_WOOTZ_WALLET_SOLANA_WEB3_JS_FOR_TEST is excluded from Android build to
// save space. Ensure this test is not build on Android. If it will be required
// to run these tests on Android, include again
// IDR_WOOTZ_WALLET_SOLANA_WEB3_JS_FOR_TEST
static_assert(!BUILDFLAG(IS_ANDROID));

using wootz_wallet::mojom::SolanaProviderError;
...
constexpr char OnAccountChangedScript[] =
    R"(async function disconnect() {await window.wootzSolana.disconnect()}
       new Promise(resolve => {
        window.wootzSolana.on('accountChanged', (result) => {
...
constexpr char CheckSolanaProviderScript[] = "!!window.wootzSolana";
...
std::string ConnectScript(const std::string& args) {
  return base::StringPrintf(
      R"(async function connect() {
          try {
            const result = await window.wootzSolana.connect(%s);
...
std::string SignTransactionScript(const std::string& args) {
  ...
  return base::StringPrintf(
      R"(async function signTransaction() {
          try {
            const result = await window.wootzSolana.signTransaction%s
...
std::string SignAllTransactionsScript(const std::string& args) {
  ...
  return base::StringPrintf(
      R"(async function signAllTransactions() {
          try {
            const result = await window.wootzSolana.signAllTransactions%s
            const isSameTx =
...
std::string SignAndSendTransactionScript(const std::string& args) {
  ...
  return base::StringPrintf(
      R"(async function signAndSendTransaction() {
          try {
            const result = await window.wootzSolana.signAndSendTransaction%s
...
std::string SignMessageScript(const std::string& args) {
  std::vector<uint8_t> signature(wootz_wallet::kSolanaSignatureSize);
  EXPECT_TRUE(wootz_wallet::Base58Decode(std::string(kTestSignature),
  ...
  return base::StringPrintf(
      R"(async function signMessage() {
          try {
            const result = await window.wootzSolana.signMessage%s
            if (JSON.stringify(result) === JSON.stringify(%s))
              return true;
            else
              return false;
...
std ::string RequestScript(const std::string& args) {
  ...
  return base::StringPrintf(
      R"(async function request() {
          try {
            const result = await window.wootzSolana.request%s
            if (JSON.stringify(result) === JSON.stringify(%s))
              return true;
...
class TestSolanaProvider final : public wootz_wallet::mojom::SolanaProvider {
 public:
  ...
  void Init(mojo::PendingRemote<wootz_wallet::mojom::SolanaEventsListener>
  ...
  void SignTransaction(wootz_wallet::mojom::SolanaSignTransactionParamPtr param,
                       SignTransactionCallback callback) override {
    EXPECT_EQ(param->encoded_serialized_msg,
              wootz_wallet::Base58Encode(kSerializedMessage));
    if (error_ == SolanaProviderError::kSuccess) {
      std::move(callback).Run(
          SolanaProviderError::kSuccess, "", kSignedTx,
          wootz_wallet::mojom::SolanaMessageVersion::kLegacy);
    } else {
      std::move(callback).Run(
  ...
  void SignAllTransactions(
      std::vector<wootz_wallet::mojom::SolanaSignTransactionParamPtr> params,
    ...
    if (error_ == SolanaProviderError::kSuccess) {
      ...
      std::move(callback).Run(
          ...
          std::vector<wootz_wallet::mojom::SolanaMessageVersion>());
      ClearError();
    }
  }
  void SignAndSendTransaction(
      wootz_wallet::mojom::SolanaSignTransactionParamPtr param,
 ...
 private:
  ...
  mojo::Remote<wootz_wallet::mojom::SolanaEventsListener> events_listener_;
};

class TestWootzContentBrowserClient : public WootzContentBrowserClient {
 public:
  TestWootzContentBrowserClient() = default;
  ~TestWootzContentBrowserClient() override = default;
  TestWootzContentBrowserClient(const TestWootzContentBrowserClient&) = delete;
  TestWootzContentBrowserClient& operator=(
      const TestWootzContentBrowserClient&) = delete;

  void RegisterBrowserInterfaceBindersForFrame(
      content::RenderFrameHost* render_frame_host,
      mojo::BinderMapWithContext<content::RenderFrameHost*>* map) override {
    WootzContentBrowserClient::RegisterBrowserInterfaceBindersForFrame(
        render_frame_host, map);
    // override binding for SolanaProvider
    map->Add<wootz_wallet::mojom::SolanaProvider>(
        base::BindRepeating(&TestWootzContentBrowserClient::BindSolanaProvider,
 ...
 private:
  void BindSolanaProvider(
    ...
    provider->set_connection_error_handler(base::BindOnce(
        &TestWootzContentBrowserClient::OnDisconnect,
  ...
  base::flat_map<
      ...
      mojo::SelfOwnedReceiverRef<wootz_wallet::mojom::SolanaProvider>>
      provider_map_;
  base::WeakPtrFactory<TestWootzContentBrowserClient> weak_ptr_factory_{this};
};
...
class SolanaProviderRendererTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    wootz_wallet::SetDefaultSolanaWallet(
    ...
    // TestWootzContentBrowserClient::RegisterBrowserInterfaceBindersForFrame
    ASSERT_TRUE(
        ui_test_utils::NavigateToURL(browser(), GURL("wootz://settings")));
    ...
    if (g_provider_solana_web3_script->empty()) {
      *g_provider_solana_web3_script = wootz_wallet::LoadDataResource(
          IDR_WOOTZ_WALLET_SOLANA_WEB3_JS_FOR_TEST);
    }
  }
  ...
  wootz_wallet::KeyringService* GetKeyringService() {
    return wootz_wallet::WootzWalletServiceFactory::GetServiceForContext(
               browser()->profile())
        ->keyring_service();
  }
 ...
 protected:
  ...
  TestWootzContentBrowserClient test_content_browser_client_;
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, DefaultWallet) {
  ...
  wootz_wallet::SetDefaultSolanaWallet(
      browser()->profile()->GetPrefs(),
      wootz_wallet::mojom::DefaultWallet::None);
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest,
  ...
  constexpr char kEvalIsWootzWallet[] = "window.solana.isWootzWallet";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, NonWritable) {
  for (const std::string& provider : {"wootzSolana", "solana"}) {
    // window.wootzSolana.* and window.solana.* (methods)
    ...
    // window.wootzSolana.* and window.solana.* (properties)
    for (const std::string& property :
         {"isPhantom", "isWootzWallet", "isConnected", "publicKey"}) {
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, IsPhantomAndIsWootzWallet) {
  ASSERT_TRUE(ExecJs(web_contents(browser()),
                     "window.wootzSolana.isPhantom = 123; "
                     "window.wootzSolana.isWootzWallet = 456"));
  // Both are non-writable
  auto result1 =
      EvalJs(web_contents(browser()), "window.wootzSolana.isPhantom");
  EXPECT_EQ(base::Value(true), result1.value);
  auto result2 =
      EvalJs(web_contents(browser()), "window.wootzSolana.isWootzWallet");
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, OnConnect) {
  auto result =
      EvalJs(web_contents(browser()),
             R"(async function connect() {await window.wootzSolana.connect()}
                new Promise(resolve => {
                  window.wootzSolana.on(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, IsConnected) {
  auto result =
      EvalJs(web_contents(browser()), "window.wootzSolana.isConnected");
  EXPECT_EQ(base::Value(true), result.value);
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, GetPublicKey) {
  auto result = EvalJs(web_contents(browser()),
                       "window.wootzSolana.publicKey.toString()");
  EXPECT_EQ(base::Value(kTestPublicKey), result.value);
}
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, Disconnect) {
  auto result = EvalJs(web_contents(browser()),
                  ...
                  const result = await window.wootzSolana.disconnect()
                  if (result == undefined)
                    return true;
                  else
                    return false;
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, NonConfigurable) {
  ...
  EXPECT_TRUE(content::EvalJs(web_contents(browser()),
                              NonConfigurableScript("wootzSolana"))
                  .ExtractBool());
  EXPECT_TRUE(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, Iframe3P) {
  constexpr char kEvalSolanaUndefined[] =
      R"(typeof window.wootzSolana === 'undefined')";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderRendererTest, SecureContextOnly) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  constexpr char kEvalSolana[] = "typeof window.wootzSolana !== 'undefined'";
  content::RenderFrameHost* main_frame =
      web_contents(browser())->GetPrimaryMainFrame();
