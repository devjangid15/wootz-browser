...
const char kPasswordWootz[] = "wootz";
const char kPasswordWootz123[] = "wootz123";
...
class KeyringServiceUnitTest : public testing::Test {
 ...
 protected:
  ...
  static std::optional<std::string> EncodePrivateKeyForExport(
      KeyringService* service,
      mojom::AccountIdPtr account_id,
      const std::string& password = kPasswordWootz) {
...
TEST_F(KeyringServiceUnitTest, SetPrefForKeyring) {
  ...
  const auto& keyrings_pref = GetPrefs()->GetDict(kWootzWalletKeyrings);
...
TEST_F(KeyringServiceUnitTest, UnlockResumesDefaultKeyring) {
  ...
  {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    ASSERT_TRUE(CreateWallet(&service, "wootz"));
    ASSERT_TRUE(AddAccount(&service, mojom::CoinType::ETH,
                           mojom::kDefaultKeyringId, "Account2"));

    salt = GetPrefs()->GetString(kWootzWalletEncryptorSalt);
    mnemonic = GetPrefs()->GetDict(kWootzWalletMnemonic).Clone();
  }
  {
    // KeyringService is now destructed, simulating relaunch
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    EXPECT_TRUE(Unlock(&service, "wootz"));
    ASSERT_FALSE(service.IsLockedSync());

    EXPECT_EQ(GetPrefs()->GetString(kWootzWalletEncryptorSalt), salt);
    EXPECT_EQ(GetPrefs()->GetDict(kWootzWalletMnemonic), mnemonic);
    EXPECT_EQ(
  ...
  }
  {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    // wrong password
    EXPECT_FALSE(Unlock(&service, "wootz123"));
...
TEST_F(KeyringServiceUnitTest, UnlockResumesNewKeyring) {
  ...
  {
    ScopedDictPrefUpdate keyrings_update(GetPrefs(), kWootzWalletKeyrings);
...
TEST_F(KeyringServiceUnitTest, GetWalletMnemonic) {
  ...
  GetPrefs()->SetBoolean(kWootzWalletKeyringEncryptionKeysMigrated, true);
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());

  // no pref exists yet
  EXPECT_EQ(GetWalletMnemonic(kPasswordWootz, &service), std::nullopt);

  AccountUtils(&service).CreateWallet(kMnemonicDivideCruise, kPasswordWootz);
  EXPECT_EQ(GetWalletMnemonic(kPasswordWootz, &service), kMnemonicDivideCruise);
  ...
  ASSERT_FALSE(Unlock(&service, kPasswordWootz123));
  ASSERT_TRUE(service.IsLockedSync());

  EXPECT_EQ(GetWalletMnemonic(kPasswordWootz, &service), std::nullopt);

  ASSERT_TRUE(Unlock(&service, kPasswordWootz));
  ASSERT_FALSE(service.IsLockedSync());

  // Can only get mnemonic when password is correct.
  EXPECT_EQ(GetWalletMnemonic(kPasswordWootz123, &service), std::nullopt);
  EXPECT_EQ(GetWalletMnemonic(kPasswordWootz, &service), kMnemonicDivideCruise);
}
...
TEST_F(KeyringServiceUnitTest, ValidatePassword) {
  ...
  EXPECT_TRUE(ValidatePassword(&service, "wootz"));
  EXPECT_FALSE(service.IsLockedSync());
  EXPECT_FALSE(ValidatePassword(&service, "wootz123"));
  EXPECT_FALSE(service.IsLockedSync());
...
TEST_F(KeyringServiceUnitTest, LockAndUnlock) {
  ...
  }
  {
    ...
    AccountUtils(&service).CreateWallet(GenerateMnemonic(16), kPasswordWootz);
    ASSERT_TRUE(AddAccount(&service, mojom::CoinType::ETH,
...
TEST_F(KeyringServiceUnitTest, Reset) {
  ...
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kWootzWalletKeyrings));
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kWootzWalletEncryptorSalt));
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kWootzWalletMnemonic));
  EXPECT_TRUE(GetPrefs()->HasPrefPath(kWootzWalletLegacyEthSeedFormat));

  EXPECT_CALL(observer, WalletReset());
  service.Reset();
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kWootzWalletKeyrings));
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kWootzWalletEncryptorSalt));
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kWootzWalletMnemonic));
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kWootzWalletLegacyEthSeedFormat));
  EXPECT_FALSE(service.GetHDKeyringById(mojom::kDefaultKeyringId));
...
TEST_F(KeyringServiceUnitTest, AccountMetasForKeyring) {
  ...
  ASSERT_TRUE(RestoreWallet(&service, kMnemonicDivideCruise, "wootz", false));
...
TEST_F(KeyringServiceUnitTest, CreateAndRestoreWallet) {
  ...
  auto verify_restore_wallet = base::BindLambdaForTesting(
      [&mnemonic_to_be_restored, &service, &address0]() {
        EXPECT_TRUE(
            RestoreWallet(&service, *mnemonic_to_be_restored, "wootz1", false));
        {
          std::vector<mojom::AccountInfoPtr> account_infos =
              service.GetAccountInfosForKeyring(mojom::kDefaultKeyringId);
          EXPECT_EQ(account_infos.size(), 1u);
...
TEST_F(KeyringServiceUnitTest, DefaultSolanaAccountCreated) {
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  ASSERT_NE(CreateWallet(&service, "wootz"), std::nullopt);
...
TEST_F(KeyringServiceUnitTest, SetDefaultKeyringImportedAccountName) {
  ...
  ASSERT_TRUE(CreateWallet(&service, kPasswordWootz));
...
TEST_F(KeyringServiceUnitTest, RestoreLegacyWootzWallet) {
  ...
  WootzWalletService wootz_wallet_service(
      shared_url_loader_factory(), TestWootzWalletServiceDelegate::Create(),
      GetPrefs(), GetLocalState());
  KeyringService& service = *wootz_wallet_service.keyring_service();
  auto verify_restore_wallet = base::BindLambdaForTesting(
      [&service](const char* mnemonic, const char* address, bool is_legacy,
                 bool expect_result) {
        service.Reset();
        EXPECT_EQ(expect_result,
                  RestoreWallet(&service, mnemonic, "wootz1", is_legacy));
        if (expect_result) {
          std::vector<mojom::AccountInfoPtr> account_infos =
              service.GetAccountInfosForKeyring(mojom::kDefaultKeyringId);
          ...
          // legacy_wootz_wallet pref so it will use the right seed
          service.Lock();
          EXPECT_TRUE(Unlock(&service, "wootz1"));
          account_infos.clear();
          account_infos =
              service.GetAccountInfosForKeyring(mojom::kDefaultKeyringId);
          ASSERT_EQ(account_infos.size(), 1u);
  ...
  // wootz legacy mnemonic can only be 24 words
...
TEST_F(KeyringServiceUnitTest, HardwareAccounts) {
  ...
  for (const auto& account : accounts) {
    ...
    ASSERT_TRUE(
        GetPrefs()->GetDict(kWootzWalletKeyrings).FindByDottedPath(path));
  }
  {
...
TEST_F(KeyringServiceUnitTest, AutoLock) {
  KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
  std::optional<std::string> mnemonic = CreateWallet(&service, "wootz");
  ...
  service.Reset();
  ASSERT_TRUE(RestoreWallet(&service, *mnemonic, "wootz", false));
  ASSERT_FALSE(service.IsLockedSync());
  ...
  ASSERT_FALSE(service.IsLockedSync());
  task_environment_.FastForwardBy(base::Minutes(4));
  GetPrefs()->SetInteger(kWootzWalletAutoLockMinutes, 3);
  ...
  task_environment_.FastForwardBy(base::Minutes(2));
  EXPECT_FALSE(service.IsLockedSync());
  GetPrefs()->SetInteger(kWootzWalletAutoLockMinutes, 10);
  task_environment_.FastForwardBy(base::Minutes(9));
  EXPECT_FALSE(service.IsLockedSync());
  task_environment_.FastForwardBy(base::Minutes(1));
  EXPECT_TRUE(service.IsLockedSync());
...
TEST_F(KeyringServiceUnitTest, SetSelectedAccount) {
  ...
  EXPECT_CALL(observer, SelectedDappAccountChanged(mojom::CoinType::SOL, _))
      .Times(0);
  EXPECT_TRUE(
      RemoveAccount(&service, imported_account->account_id, kPasswordWootz));
  EXPECT_TRUE(Lock(&service));
  EXPECT_EQ(first_account, service.GetSelectedWalletAccount());
  EXPECT_EQ(first_account, service.GetSelectedEthereumDappAccount());
  observer.WaitAndVerify();
...
TEST_F(KeyringServiceUnitTest, ImportBitcoinAccount) {
  ...
  EXPECT_CALL(observer, AccountsChanged());
  EXPECT_TRUE(RemoveAccount(&service, acc1->account_id, kPasswordWootz));
  observer.WaitAndVerify();

  EXPECT_CALL(observer, AccountsChanged());
  EXPECT_TRUE(RemoveAccount(&service, acc2->account_id, kPasswordWootz));
  observer.WaitAndVerify();

  EXPECT_CALL(observer, AccountsChanged());
  EXPECT_TRUE(RemoveAccount(&service, acc3->account_id, kPasswordWootz));
  observer.WaitAndVerify();
...
TEST_F(KeyringServiceUnitTest, SolanaKeyring) {
  {
    ...
    service.Lock();
    ASSERT_TRUE(Unlock(&service, "wootz"));
...
class KeyringServiceAccountDiscoveryUnitTest : public KeyringServiceUnitTest {
 public:
  ...
  void PrepareAccounts(mojom::CoinType coin_type, mojom::KeyringId keyring_id) {
    KeyringService service(json_rpc_service(), GetPrefs(), GetLocalState());
    saved_mnemonic_ = CreateWallet(&service, "wootz").value_or("");
...
TEST_F(KeyringServiceAccountDiscoveryUnitTest, AccountDiscovery) {
  ...
  BitcoinTestRpcServer bitcoin_test_rpc_server(
      wootz_wallet_service.GetBitcoinWalletService());
  ...
  EXPECT_TRUE(RestoreWallet(&service, saved_mnemonic(), "wootz1", false));
  observer.WaitAndVerify();
  std::vector<mojom::AccountInfoPtr> account_infos =
      service.GetAccountInfosForKeyring(mojom::kDefaultKeyringId);
...
TEST_F(KeyringServiceUnitTest, AccountsAdded) {
  ...
  RestoreWallet(&service, kMnemonicDivideCruise, kPasswordWootz, false);
  observer.WaitAndVerify();
...
TEST_F(KeyringServiceUnitTest, GetBitcoinAddresses) {
  ...
  ASSERT_TRUE(RestoreWallet(&service, kMnemonicAbandonAbandon, "wootz", false));
...
TEST_F(KeyringServiceUnitTest, MigrateSelectedAccount) {
  ...
  ASSERT_TRUE(
      RestoreWallet(service.get(), kMnemonicDivideCruise, "wootz", false));
  ...
  GetPrefs()->ClearPref(kWootzWalletSelectedWalletAccount);
  GetPrefs()->ClearPref(kWootzWalletSelectedSolDappAccount);
  GetPrefs()->ClearPref(kWootzWalletSelectedEthDappAccount);
  GetPrefs()->SetInteger(kWootzWalletSelectedCoinDeprecated,
                         static_cast<int>(mojom::CoinType::FIL));
  auto keyrings = GetPrefs()->GetDict(kWootzWalletKeyrings).Clone();
  ...
  GetPrefs()->SetDict(kWootzWalletKeyrings, std::move(keyrings));
  ...
  EXPECT_FALSE(GetPrefs()->HasPrefPath(kWootzWalletSelectedCoinDeprecated));
  keyrings = GetPrefs()->GetDict(kWootzWalletKeyrings).Clone();
...
TEST_F(KeyringServiceUnitTest, GetOrchardRawBytes_ZCashDisabled) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndDisableFeature(features::kWootzWalletZCashFeature);
