...
namespace {
int GetStatusTitle(wootz_wallet::mojom::TransactionStatus status) {
  switch (status) {
    case wootz_wallet::mojom::TransactionStatus::Confirmed:
      return IDS_WALLET_TRANSACTION_STATUS_UPDATE_MESSAGE_TITLE_CONFIRMED;
    case wootz_wallet::mojom::TransactionStatus::Error:
      return IDS_WALLET_TRANSACTION_STATUS_UPDATE_MESSAGE_TITLE_ERROR;
    case wootz_wallet::mojom::TransactionStatus::Dropped:
...
void PushNotification(content::BrowserContext* context,
  ...
  auto notification = CreateMessageCenterNotification(
      title, body, uuid,
      GURL("wootzapp://wallet/crypto/accounts/" + from + "#" + uuid));
...
WalletNotificationService::WalletNotificationService(
  ...
  if (wootz_wallet_service) {
    wootz_wallet_service->tx_service()->AddObserver(
