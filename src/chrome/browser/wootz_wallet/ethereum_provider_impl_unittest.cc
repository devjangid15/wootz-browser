...
class TestEventsListener : public wootz_wallet::mojom::EventsListener {
 public:
  ...
  mojo::PendingRemote<wootz_wallet::mojom::EventsListener> GetReceiver() {
 ...
 private:
  mojo::Receiver<wootz_wallet::mojom::EventsListener> observer_receiver_{this};
};
...
class EthereumProviderImplUnitTest : public testing::Test {
 public:
  ...
  void TearDown() override {
    ...
    WootzWalletServiceDelegateImpl::SetActiveWebContentsForTesting(nullptr);
  }
  ...
  void SetUp() override {
    ...
    WootzWalletServiceDelegateImpl::SetActiveWebContentsForTesting(
        web_contents_.get());
    ...
    asset_ratio_service_->SetAPIRequestHelperForTesting(
        shared_url_loader_factory_);
    wootz_wallet_service_ = std::make_unique<WootzWalletService>(
        shared_url_loader_factory_,
        WootzWalletServiceDelegate::Create(browser_context()), prefs(),
        local_state_->Get());
    ASSERT_TRUE(wootz_wallet_service_.get());
    json_rpc_service_ = wootz_wallet_service_->json_rpc_service();
    json_rpc_service_->SetAPIRequestHelperForTesting(
        shared_url_loader_factory_);
    SetNetwork(mojom::kMainnetChainId, std::nullopt);
    keyring_service_ = wootz_wallet_service_->keyring_service();
    tx_service_ = wootz_wallet_service_->tx_service();
    ...
    provider_ = std::make_unique<EthereumProviderImpl>(
        host_content_settings_map(), wootz_wallet_service_.get(),
  ...
  void Unlock() {
    ...
    keyring_service_->Unlock(
        "wootz", base::BindLambdaForTesting([&run_loop](bool success) {
          EXPECT_TRUE(success);
          run_loop.Quit();
        }));
    run_loop.Run();
  ...
  void CreateWootzWalletTabHelper() {
  ...
  void AddEthereumPermission(const mojom::AccountIdPtr& account_id) {
    EXPECT_TRUE(permissions::WootzWalletPermissionContext::AddPermission(
        blink::PermissionType::WOOTZ_ETHEREUM, browser_context(), GetOrigin(),
        account_id->address));
  }

  void ResetEthereumPermission(const mojom::AccountIdPtr& account_id) {
    base::RunLoop run_loop;
    wootz_wallet_service_->ResetPermission(
        account_id.Clone(), base::BindLambdaForTesting([&](bool success) {
          EXPECT_TRUE(success);
          run_loop.Quit();
        }));
  ...
  void SignMessageHardware(bool user_approved,
    ...
    wootz_wallet_service_->NotifySignMessageRequestProcessed(
        user_approved, wootz_wallet_service_->sign_message_id_ - 1,
  ...
  void SignMessage(std::optional<bool> user_approved,
    ...
    if (user_approved) {
      wootz_wallet_service_->NotifySignMessageRequestProcessed(
          *user_approved, wootz_wallet_service_->sign_message_id_ - 1, nullptr,
          std::nullopt);
    }
    run_loop.Run();
  }
  ...
  GetPendingGetEncryptionPublicKeyRequests() const {
    ...
    wootz_wallet_service_->GetPendingGetEncryptionPublicKeyRequests(
        base::BindLambdaForTesting(
  ...
  std::vector<mojom::DecryptRequestPtr> GetPendingDecryptRequests() const {
    ...
    wootz_wallet_service_->GetPendingDecryptRequests(base::BindLambdaForTesting(
  ...
  void GetEncryptionPublicKey(const std::string& address,
    ...
    if (requests.size() > 0) {
      ...
      EXPECT_TRUE(wootz_wallet_tab_helper()->IsShowingBubble());
      wootz_wallet_service_->NotifyGetPublicKeyRequestProcessed(
    ...
    *unsafe_message = "";
    ...
    if (requests.size() > 0) {
      ...
      EXPECT_EQ(requests[0]->account_id,
                GetAccountUtils().FindAccountIdByAddress(address));
      EXPECT_TRUE(wootz_wallet_tab_helper()->IsShowingBubble());
      wootz_wallet_service_->NotifyDecryptRequestProcessed(
          requests[0]->request_id, approved);
    }
    run_loop.Run();
  }
  ...
  GetPendingAddSuggestTokenRequests() const {
    ...
    wootz_wallet_service_->GetPendingAddSuggestTokenRequests(
        base::BindLambdaForTesting(
 ...
 protected:
  ...
  std::unique_ptr<WootzWalletService> wootz_wallet_service_;
};
...
TEST_F(EthereumProviderImplUnitTest, EmptyDelegate) {
  EthereumProviderImpl provider_impl(host_content_settings_map(),
                                     wootz_wallet_service_.get(), nullptr,
                                     prefs());
...
TEST_F(EthereumProviderImplUnitTest, RequestEthereumPermissionsWithAccounts) {
  ...
  host_content_settings_map()->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::WOOTZ_ETHEREUM, CONTENT_SETTING_BLOCK);
  base::RunLoop run_loop;
  provider()->RequestEthereumPermissions(
      base::BindLambdaForTesting([&](base::Value id,
  ...
  // again.
  host_content_settings_map()->SetContentSettingDefaultScope(
      url, url, ContentSettingsType::WOOTZ_ETHEREUM, CONTENT_SETTING_DEFAULT);
  EXPECT_THAT(RequestEthereumPermissions(), ElementsAre(address_0, address_1));
}
...
TEST_F(EthereumProviderImplUnitTest, SignMessage) {
  CreateWallet();
  auto account_0 = GetAccountUtils().EnsureEthAccount(0);
  Navigate(GURL("https://wootz.com"));
  for (const auto& message : {
           std::string("0x1234"),
           ToHex(GetSIWEMessage("https://wootz.com", account_0->address,
                                "https://wootz.com/login", "1")),
       }) {
...
TEST_F(EthereumProviderImplUnitTest, SigninWithEthereumError) {
  ...
  mojom::ProviderError error = mojom::ProviderError::kUnknown;
  std::string error_message;
  CreateWootzWalletTabHelper();
  wootz_wallet_tab_helper()->SetSkipDelegateForTesting(true);
  Navigate(GURL("https://wootz.com"));
  AddEthereumPermission(account_0->account_id);
  ...
  struct {
    ...
    std::string provider_err_msg;
      ...
      {"https://wootz.com", account_0->address, "https://wootz.com/login",
       ...
       l10n_util::GetStringFUTF8(
           ...
           base::StrCat({l10n_util::GetStringUTF16(IDS_WOOTZ_WALLET_CHAIN_ID),
                         u": 5566"})),
       std::string("5566"), mojom::ProviderError::kInternalError,
       l10n_util::GetStringFUTF8(
           IDS_WOOTZ_WALLET_SIGN_MESSAGE_CHAIN_ID_MISMATCH, u"5566")},
      {"https://wootz.com", account_1->address, "https://wootz.com/login", "1",
       ...
       l10n_util::GetStringFUTF8(IDS_WOOTZ_WALLET_SIGN_MESSAGE_ACCOUNT_MISMATCH,
                                 base::ASCIIToUTF16(account_1->address))},
      {"https://example.com", account_0->address, "https://wootz.com/login",
       ...
       l10n_util::GetStringFUTF8(
           ...
           u"https://example.com"),
       std::nullopt, mojom::ProviderError::kInternalError,
       l10n_util::GetStringFUTF8(IDS_WOOTZ_WALLET_SIGN_MESSAGE_DOMAIN_MISMATCH,
                                 u"https://example.com")},
  };
  for (const auto& invalid_case : cases) {
    ...
    wootz_wallet_service_->NotifySignMessageErrorProcessed(
...
TEST_F(EthereumProviderImplUnitTest, SignTypedMessage) {
  ...
  SignTypedMessage(std::nullopt, address_0, "{...}", DecodeHexHash("wootz"),
                   primary_hash, domain.Clone(), nullptr, &signature, &error,
                   &error_message);
  EXPECT_TRUE(signature.empty());
  EXPECT_EQ(error, mojom::ProviderError::kInvalidParams);
...
TEST_F(EthereumProviderImplUnitTest, SignMessageRequestQueue) {
  ...
  // wrong order
  wootz_wallet_service_->NotifySignMessageRequestProcessed(true, id2, nullptr,
                                                           std::nullopt);
  EXPECT_EQ(GetSignMessageQueueSize(), 3u);
  EXPECT_EQ(GetSignMessageQueueFront()->id, id1);
  EXPECT_EQ(GetSignMessageQueueFront()
  ...
  wootz_wallet_service_->NotifySignMessageRequestProcessed(true, id3, nullptr,
                                                           std::nullopt);
  EXPECT_EQ(GetSignMessageQueueSize(), 3u);
  EXPECT_EQ(GetSignMessageQueueFront()->id, id1);
  EXPECT_EQ(GetSignMessageQueueFront()
  ...
  wootz_wallet_service_->NotifySignMessageRequestProcessed(true, id1, nullptr,
                                                           std::nullopt);
...
TEST_F(EthereumProviderImplUnitTest, AccountsChangedEvent) {
  ...
  Navigate(GURL("https://wootzsoftware.com"));
  AddEthereumPermission(account_1->account_id);
  SetSelectedAccount(account_0->account_id);
  EXPECT_FALSE(observer_->AccountsChangedFired());
}
...
TEST_F(EthereumProviderImplUnitTest, SwitchEthereumChain) {
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringFUTF8(IDS_WALLET_UNKNOWN_CHAIN,
                                      base::ASCIIToUTF16(chain_id)));
  EXPECT_FALSE(wootz_wallet_tab_helper()->IsShowingBubble());
  ...
  EXPECT_EQ(error_message,
            l10n_util::GetStringUTF8(IDS_WALLET_USER_REJECTED_REQUEST));
  EXPECT_TRUE(wootz_wallet_tab_helper()->IsShowingBubble());
  wootz_wallet_tab_helper()->CloseBubble();
  EXPECT_FALSE(wootz_wallet_tab_helper()->IsShowingBubble());
...
TEST_F(EthereumProviderImplUnitTest, AddEthereumChainSwitchesForInnactive) {
  ...
  std::string params = R"({"params": [{
        "chainId": "0xaa36a7",
        "chainName": "Sepolia",
        "rpcUrls": ["https://ethereum-sepolia.wallet.wootz.com"]
      }]})";
  base::RunLoop run_loop;
  provider()->AddEthereumChain(
      params,
