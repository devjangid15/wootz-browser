...
class TestWootzWalletServiceObserverForAssetDiscoveryManager
    : public wootz_wallet::WootzWalletServiceObserverBase {
 public:
  TestWootzWalletServiceObserverForAssetDiscoveryManager() = default;
 ...
 protected:
  void SetUp() override {
    scoped_feature_list_.InitAndEnableFeature(
        features::kNativeWootzWalletFeature);
    ...
    wallet_service_ = std::make_unique<WootzWalletService>(
        shared_url_loader_factory_,
        WootzWalletServiceDelegate::Create(profile_.get()), GetPrefs(),
        GetLocalState());
    ...
    wallet_service_observer_ = std::make_unique<
        TestWootzWalletServiceObserverForAssetDiscoveryManager>();
  ...
  std::unique_ptr<TestWootzWalletServiceObserverForAssetDiscoveryManager>
      wallet_service_observer_;
  content::BrowserTaskEnvironment task_environment_;
  std::unique_ptr<ScopedTestingLocalState> local_state_;
  std::unique_ptr<TestingProfile> profile_;
  std::unique_ptr<WootzWalletService> wallet_service_;
...
TEST_F(AssetDiscoveryManagerUnitTest, DiscoverAssetsOnAllSupportedChains) {
  ...
  // kWootzWalletLastDiscoveredAssetsAt does not update if accounts_added set
  base::Time current_assets_last_discovered_at =
      GetPrefs()->GetTime(kWootzWalletLastDiscoveredAssetsAt);
  ...
  // Verify OnDiscoverAssetsStarted and OnDiscoverAssetsCompleted both fire and
  // kWootzWalletLastDiscoveredAssetsAt updates if accounts_added not set
