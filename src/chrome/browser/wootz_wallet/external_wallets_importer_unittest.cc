...
TEST_F(ExternalWalletsImporterUnitTest, OnGetImportInfo_10K_Iterations) {
  ...
  ImportInfo info;
  ImportError error;
  SimulateGetImportInfo("wootz4ever", valid_data_10K, &result, &info, &error);
  EXPECT_TRUE(result);
...
TEST_F(ExternalWalletsImporterUnitTest, OnGetImportInfo_UTF8Mnemonic) {
  ...
  ImportInfo info;
  ImportError error;
  SimulateGetImportInfo("wootz4ever", valid_data_with_utf8_mnemonic, &result,
                        &info, &error);
  EXPECT_TRUE(result);
  EXPECT_EQ(error, ImportError::kNone);
  EXPECT_EQ(info.mnemonic, kMnemonicDripCaution);
...
TEST_F(ExternalWalletsImporterUnitTest, ImportLegacyWallet) {
  ...
  ImportInfo info;
  ImportError error;
  SimulateGetImportInfo("bbwootzy", valid_legacy_data, &result, &info, &error);
  EXPECT_TRUE(result);
  EXPECT_EQ(error, ImportError::kNone);
