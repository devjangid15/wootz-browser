...
// signMessage
constexpr char kMessage[] = "wootzy baby!";
...
class TestTxServiceObserver : public mojom::TxServiceObserver {
 public:
  ...
  mojo::PendingRemote<wootz_wallet::mojom::TxServiceObserver> GetReceiver() {
 ...
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    if (g_provider_solana_web3_script->empty()) {
      *g_provider_solana_web3_script =
          LoadDataResource(IDR_WOOTZ_WALLET_SOLANA_WEB3_JS_FOR_TEST);
    }
  }
  ...
  void UserGrantPermission(bool granted,
    ...
    if (granted) {
      ...
      permissions::WootzWalletPermissionContext::Cancel(web_contents());
    }
  ...
  bool GetIsWootzWalletViaProxy() {
    return EvalJs(web_contents(), "getIsWootzWalletViaProxy()").ExtractBool();
  }
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, ConnectRequestInProgress) {
  ...
  (async function() {
    try {
      return await window.wootzSolana.connect()
    } catch (err) {
      return err
    }
  })()
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, ConnectedStatusAndPermission) {
  ...
  host_content_settings_map()->ClearSettingsForOneTypeWithPredicate(
      ContentSettingsType::WOOTZ_SOLANA, base::Time(), base::Time::Max(),
      base::BindLambdaForTesting(
          [&url, &account_0](const ContentSettingsPattern& primary_pattern,
                             const ContentSettingsPattern& secondary_pattern) {
            url::Origin new_origin;
            if (GetSubRequestOrigin(permissions::RequestType::kWootzSolana,
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, ConnectedStatusInIframes) {
  ...
  // 2. WootzWalletProviderDelegateImpl::RenderFrameHostChanged doesn't get
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, GetPublicKey) {
  ...
  constexpr char get_public_key_script[] =
      "window.wootzSolana.publicKey ? window.wootzSolana.publicKey.toString() "
      ": ''";
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, SignAllTransactions) {
  ...
  EXPECT_TRUE(WaitForWalletBubble(web_contents()));
  // user rejected request
  wootz_wallet_service_->NotifySignAllTransactionsRequestProcessed(
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, Request) {
  ...
  wootz_wallet_service_->NotifySignMessageRequestProcessed(true, 0, nullptr,
                                                           std::nullopt);
  WaitForResultReady();
  ...
  EXPECT_TRUE(WaitForWalletBubble(web_contents()));
  wootz_wallet_service_->NotifySignTransactionRequestProcessed(true, 0, nullptr,
                                                               std::nullopt);
  WaitForResultReady();
...
IN_PROC_BROWSER_TEST_F(SolanaProviderTest, CallViaProxy) {
  ...
  EXPECT_TRUE(GetIsWootzWalletViaProxy());
