...
class WootzWalletSignMessageBrowserTest : public InProcessBrowserTest {
 public:
  WootzWalletSignMessageBrowserTest()
      : https_server_(net::EmbeddedTestServer::TYPE_HTTPS) {
    scoped_feature_list_.InitAndEnableFeature(
        wootz_wallet::features::kNativeWootzWalletFeature);
  }

  ~WootzWalletSignMessageBrowserTest() override = default;
...
IN_PROC_BROWSER_TEST_F(WootzWalletSignMessageBrowserTest, UserApprovedRequest) {
  RestoreWallet();
...
IN_PROC_BROWSER_TEST_F(WootzWalletSignMessageBrowserTest, UserRejectedRequest) {
  RestoreWallet();
  GURL url = https_server()->GetURL("a.com", "/sign_message.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_TRUE(WaitForLoadStop(web_contents()));
...
IN_PROC_BROWSER_TEST_F(WootzWalletSignMessageBrowserTest, UnknownAddress) {
  RestoreWallet();
  GURL url = https_server()->GetURL("a.com", "/sign_message.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_TRUE(WaitForLoadStop(web_contents()));
...
IN_PROC_BROWSER_TEST_F(WootzWalletSignMessageBrowserTest, InvalidAddressParam) {
  RestoreWallet();
  GURL url = https_server()->GetURL("a.com", "/sign_message.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_TRUE(WaitForLoadStop(web_contents()));
...
IN_PROC_BROWSER_TEST_F(WootzWalletSignMessageBrowserTest, NoEthPermission) {
  RestoreWallet();
  GURL url = https_server()->GetURL("a.com", "/sign_message.html");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
  EXPECT_TRUE(WaitForLoadStop(web_contents()));
...
IN_PROC_BROWSER_TEST_F(WootzWalletSignMessageBrowserTest, SIWE) {
  RestoreWallet();
  GURL url = https_server()->GetURL("a.com", "/sign_message.html");
