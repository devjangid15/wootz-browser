...
import org.chromium.wootz_wallet.mojom.AddSuggestTokenRequest;
...
public class AddTokenFragment extends BaseDAppsFragment {
    ...
    private void notifyAddSuggestTokenRequestProcessed(boolean approved) {
        ...
        getWootzWalletService().notifyAddSuggestTokenRequestsProcessed(approved, addresses);
        initComponents(false);
    }

    private void fillAddSuggestTokenRequest(boolean init) {
        if (mWalletModel == null) return;
        getWootzWalletService().getPendingAddSuggestTokenRequests(requests -> {
            ...
            if (init) {
                ...
                mTokenAddress.setOnClickListener(v -> {
                    ...
                    if (activity instanceof WootzWalletBaseActivity && selectedNetwork != null) {
                        Utils.openAddress(
                                "/token/" + mCurrentAddSuggestTokenRequest.token.contractAddress,
                                (WootzWalletBaseActivity) activity,
