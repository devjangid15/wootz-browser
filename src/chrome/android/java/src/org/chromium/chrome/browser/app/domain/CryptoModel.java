...
public class CryptoModel {
    ...
    public CryptoModel(Context context, TxService txService, KeyringService keyringService,
            ...
            WootzWalletService wootzWalletService, AssetRatioService assetRatioService,
            CryptoSharedActions wootzSharedActions, SwapService swapService) {
        mContext = context;
        ...
        mNetworkModel =
                new NetworkModel(
                        mWootzWalletService, mJsonRpcService, mSharedData, mCryptoSharedActions);
    }
    ...
    public void resetServices(
            ...
            WootzWalletService mWootzWalletService,
        ...
        synchronized (mLock) {
            ...
            mNetworkModel.resetServices(mWootzWalletService, mJsonRpcService);
        }
        init();
    }
    ...
    public List<CryptoAccountTypeInfo> getSupportedCryptoAccountTypes() {
        List<CryptoAccountTypeInfo> wootzAccountTypeInfos = new ArrayList<>();
        wootzAccountTypeInfos.add(new CryptoAccountTypeInfo(
                mContext.getString(R.string.wootz_wallet_create_account_ethereum_description),
                mContext.getString(R.string.wallet_eth_name), CoinType.ETH, R.drawable.eth));

        wootzAccountTypeInfos.add(
                new CryptoAccountTypeInfo(
                        mContext.getString(R.string.wootz_wallet_create_account_solana_description),
        ...
        wootzAccountTypeInfos.add(
                new CryptoAccountTypeInfo(
                        mContext.getString(
                                R.string.wootz_wallet_create_account_filecoin_description),
                        mContext.getString(R.string.wallet_fil_name),
                        CoinType.FIL,
                        R.drawable.ic_fil_asset_icon));
        return wootzAccountTypeInfos;
    }
    ...
    public void updateNftDiscovery(boolean isEnabled) {
        mWootzWalletService.setNftDiscoveryEnabled(isEnabled);
    }

    public void isNftDiscoveryEnabled(Callback1<Boolean> callback) {
        mWootzWalletService.getNftDiscoveryEnabled(
    ...
    class CryptoSharedDataImpl implements CryptoSharedData {
        ...
        public String getChainId() {
            if (mNetworkModel.mChainId.getValue() == null) {
                return WootzWalletConstants.MAINNET_CHAIN_ID;
            }
