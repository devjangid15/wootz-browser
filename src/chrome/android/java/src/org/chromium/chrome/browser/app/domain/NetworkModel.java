...
import org.chromium.wootz_wallet.mojom.JsonRpcServiceObserver;
...
public class NetworkModel implements JsonRpcServiceObserver {
    ...
    public NetworkModel(
        ...
        mCryptoActions = wootzSharedActions;

        _mChainId = new MediatorLiveData<>();
        _mChainId.setValue(WootzWalletConstants.MAINNET_CHAIN_ID);
        mChainId = _mChainId;
        ...
        _mDefaultNetwork.addSource(
                ...
                chainIdAndInfosPair -> {
                    ...
                    if (chainId == null || wootzNetworks == null) return;
                    for (NetworkInfo networkInfo : wootzNetworks) {
    ...
    private void updateChainId() {
        if (mMode == Mode.WALLET_MODE) {
            ...
            mWootzWalletService.getNetworkForSelectedAccountOnActiveOrigin(networkInfo -> {
                if (networkInfo != null) {
                    _mChainId.postValue(networkInfo.chainId);
                }
            });
    ...
    public void resetServices(
            WootzWalletService wootzWalletService, JsonRpcService jsonRpcService) {
        synchronized (mLock) {
            mWootzWalletService = wootzWalletService;
            mJsonRpcService = jsonRpcService;
        }
    ...
    public void init() {
        synchronized (mLock) {
            ...
            for (Map.Entry<String, Integer> entry :
                ...
                if (!AndroidUtils.isDebugBuild()
                        && entry.getKey().equals(WootzWalletConstants.LOCALHOST_CHAIN_ID)) {
            ...
            getAllNetworks(
                    mJsonRpcService,
                    wootzNetworks -> {
                        _mCryptoNetworks.postValue(wootzNetworks);
    ...
    public void setNetworkWithAccountCheck(NetworkInfo networkToBeSetAsSelected,
        ...
        mWootzWalletService.ensureSelectedAccountForChain(
    ...
    public void setNetworkForSelectedAccountOnActiveOrigin(
        ...
        mWootzWalletService.setNetworkForSelectedAccountOnActiveOrigin(
    ...
    public NetworkInfo getNetwork(String chainId) {
        ...
        List<NetworkInfo> wootzNws = JavaUtils.safeVal(_mCryptoNetworks.getValue());
        for (NetworkInfo info : wootzNws) {
