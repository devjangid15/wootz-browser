...
import org.chromium.chrome.browser.wootz_wallet.modal.DAppsDialog;
...
public class DAppsWalletController implements ConnectionErrorHandler {
    ...
    private WootzWalletPanel mWootzWalletPanel;
        ...
        mDefaultLifecycleObserver =
                new DefaultLifecycleObserver() {
                    ...
                    public void onResume(@NonNull LifecycleOwner owner) {
                        if (mWootzWalletPanel != null) {
                            mWootzWalletPanel.resume();
                        }
                    }
    ...
    private void createAndShowWalletPanel() {
        boolean showExpandButton =
                mVisibleUrl != null && !mVisibleUrl.getHost().equals(WOOTZ_WALLET_HOST);
        mWootzWalletPanel =
                new WootzWalletPanel(
                        mAnchorViewHost, mDialogOrPanelDismissListener, showExpandButton);
        mWootzWalletPanel.showLikePopDownMenu();
    ...
    public void dismiss() {
        if (isShowingPanel()) {
            mWootzWalletPanel.dismiss();
        }
        if (isShowingDialog()) {
            mDAppsDialog.dismiss();
        }
        mWootzWalletPanel = null;
        mDAppsDialog = null;
        cleanUp();
    }

    public boolean isShowingPanel() {
        return mWootzWalletPanel != null && mWootzWalletPanel.isShowing();
    }
