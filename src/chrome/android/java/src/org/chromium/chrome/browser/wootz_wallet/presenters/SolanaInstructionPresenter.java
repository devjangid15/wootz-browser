...
package org.chromium.chrome.browser.wootz_wallet.presenters;

import android.content.Context;
import android.text.TextUtils;

import org.chromium.wootz_wallet.mojom.CoinType;
import org.chromium.wootz_wallet.mojom.SolanaAccountMeta;
import org.chromium.wootz_wallet.mojom.SolanaInstruction;
import org.chromium.wootz_wallet.mojom.SolanaInstructionAccountParam;
import org.chromium.wootz_wallet.mojom.SolanaInstructionParam;
import org.chromium.chrome.R;
import org.chromium.chrome.browser.wootz_wallet.adapters.TwoLineItemRecyclerViewAdapter.TwoLineItem;
import org.chromium.chrome.browser.wootz_wallet.adapters.TwoLineItemRecyclerViewAdapter.TwoLineItemHeader;
...
public class SolanaInstructionPresenter {
    ...
    public List<TwoLineItem> toTwoLineList(Context context) {
        ...
        var txSubType =
                context.getString(TransactionUtils.getSolTxSubType(mSolanaInstruction.programId,
                        mSolanaInstruction.decodedData != null
                                ? mSolanaInstruction.decodedData.instructionType
                                : -1));
        if (!txSubType.equals(context.getString(R.string.wootz_wallet_unknown))) {
        ...
        if (shouldShowRawData()) {
            ...
            twoLineItems.add(
                    new TwoLineItemHeader(context.getString(R.string.wootz_wallet_data_text)));
            twoLineItems.addAll(solanaInstructionPresenter.dataToList());

            twoLineItems.add(
                    new TwoLineItemHeader(context.getString(R.string.wootz_wallet_tx_progam_id)));
            ...
            for (SolanaAccountMeta solanaAccountMeta : getSolanaInstruction().accountMetas) {
                ...
                if (lookupTableIndex != null) {
                    twoLineItemDataSources.add(new TwoLineItemText(
                            resources.getString(
                                    R.string.wootz_wallet_solana_address_lookup_table_account),
                            null));
                }
                ...
                if (lookupTableIndex != null) {
                    twoLineItemDataSources.add(new TwoLineItemText(
                            resources.getString(
                                    R.string.wootz_wallet_solana_address_lookup_table_index),
            ...
            for (SolanaInstructionAccountPresenter accountPresenter : mAccountDatas) {
                ...
                if (!accountPresenter.mSolanaAccountMeta.isSigner
                    ...
                    twoLineItemDataSources.add(new TwoLineItemText(
                            resources.getString(
                                    R.string.wootz_wallet_solana_address_lookup_table_index),
                            String.valueOf(Byte.toUnsignedInt(accountPresenter.mSolanaAccountMeta
                                                                      .addrTableLookupIndex.val))));
                }
            }
        }
